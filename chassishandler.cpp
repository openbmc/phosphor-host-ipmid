...
constexpr auto settingsRoot = "/";
constexpr auto settingsMatch = "host0";

constexpr auto ipInterface = "xyz.openbmc_project.Network.IP";
constexpr auto macInterface = "xyz.openbmc_project.Network.MACAddress";
...
int getHostNetworkData(get_sys_boot_options_response_t* respptr)
{
    ...
    try
    {
        ...
        //  as settingsMatch.
        //  Later settingsMatch will be replaced with busname.

        sdbusplus::bus::bus bus(ipmid_get_sd_bus_connection());

        auto ipObjectInfo =
            ipmi::getDbusObject(bus, ipInterface, settingsRoot, settingsMatch);

        auto macObjectInfo =
            ipmi::getDbusObject(bus, macInterface, settingsRoot, settingsMatch);

        properties = ipmi::getAllDbusProperties(
            bus, ipObjectInfo.second, ipObjectInfo.first, ipInterface);
        auto variant = ipmi::getDbusProperty(bus, macObjectInfo.second,
                                             macObjectInfo.first, macInterface,
                                             "MACAddress");
...
bool predicateOffset(uint8_t offsetOne, uint8_t offsetTwo)
{
    return (offsetOne != offsetTwo);
}

int setHostNetworkData(std::vector<uint8_t>& data)
{
    ...
    uint8_t prefix{0};
    uint8_t zeroCookie[sizeCookie] = {0, 0, 0, 0};
    ...
    try
    {
        do
        {
            ...
            if (std::equal((data.begin() + cookieOffset),
                           (data.begin() + cookieOffset + sizeCookie),
            ...
            // vesion == 0x00 0x01
            if (std::equal((data.begin() + versionOffset),
                           (data.begin() + versionOffset + sizeVersion),
            ...
            if (!data.empty())
            {
                std::copy((data.begin() + macOffset),
                          (data.begin() + macOffset + 5),
                          std::ostream_iterator<int>(result, ":"));
                mac = result.str();
            }

            std::copy(data.begin() + addrTypeOffset,
                      data.begin() + addrTypeOffset +
            ...
            std::copy(
                data.begin() + addrSizeOffset,
                (data.begin() + addrSizeOffset + sizeof(decltype(addrSize))),
                &addrSize);
            uint8_t prefixOffset = ipAddrOffset + addrSize;

            std::copy(data.begin(),
                      (data.begin() + prefixOffset + sizeof(decltype(prefix))),
                      &prefix);
            uint8_t gatewayOffset = prefixOffset + sizeof(decltype(prefix));
            ...
            ipAddress = getAddrStr(family, data.data(), ipAddrOffset, addrSize);

            gateway = getAddrStr(family, data.data(), gatewayOffset, addrSize);
        ...
        ipmi::setDbusProperty(bus, ipObjectInfo.second, ipObjectInfo.first,
                              ipInterface, "Address", std::string(ipAddress));
        ipmi::setDbusProperty(bus, ipObjectInfo.second, ipObjectInfo.first,
                              ipInterface, "PrefixLength", prefix);
        ipmi::setDbusProperty(bus, ipObjectInfo.second, ipObjectInfo.first,
                              ipInterface, "Origin", addressOrigin);
        ipmi::setDbusProperty(bus, ipObjectInfo.second, ipObjectInfo.first,
                              ipInterface, "Gateway", std::string(gateway));
        ipmi::setDbusProperty(
            bus, ipObjectInfo.second, ipObjectInfo.first, ipInterface, "Type",
            std::string("xyz.openbmc_project.Network.IP.Protocol.IPv4"));
        ipmi::setDbusProperty(bus, macObjectInfo.second, macObjectInfo.first,
                              macInterface, "MACAddress", std::string(mac));
                                             ...
                                             std::vector<uint8_t>& data)
{
    ...
    if (parameter == (uint8_t)BootOptionParameter::bootFlags)
    {
        IpmiValue bootOption = ((data[1] & 0x3C) >> 2);
        using namespace chassis::internal;
        using namespace chassis::internal::cache;
        ...
        try
        {
            bool permanent = (data[0] & setParmBootFlagsPermanent) ==
    ...
    else if (parameter == (uint8_t)BootOptionParameter::opalNetworkSettings)
    {

        int ret = setHostNetworkData(data);
        if (ret < 0)
        {
            log<level::ERR>(
                "ipmiChassisSetSysBootOptions: Error in setHostNetworkData");
            return ipmi::responseUnspecifiedError();
        }
    }
    else if (parameter == static_cast<uint8_t>(BootOptionParameter::bootInfo))
    {
    ...
    {
        log<level::ERR>("ipmiChassisSetSysBootOptions: Unsupported parameters",
                        entry("PARAM=0x%x", parameter));
