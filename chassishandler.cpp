...
// Defines
#define setParmBootFlagsPermanent 0x40
#define setParmBootFlagsValidOneTime 0x80
#define setParmBootFlagsValidPermanent 0xC0
...
constexpr size_t sizeBootOption = (uint8_t)
...
{
    uint8_t parameter;
    uint8_t data[sizeBootOption];
} __attribute__((packed));

int getHostNetworkData(uint8_t* version, uint8_t* parm, uint8_t* data)
{
    ...
    try
    {
        ...
        {
            std::memset(data, 0, sizeBootOption);
            rc = -1;
            return rc;
        }
        ...
        sscanf(MACAddress.c_str(), ipmi::network::MAC_ADDRESS_FORMAT,
               (data + MAC_OFFSET), (data + MAC_OFFSET + 1),
               (data + MAC_OFFSET + 2), (data + MAC_OFFSET + 3),
               (data + MAC_OFFSET + 4), (data + MAC_OFFSET + 5));

        data[MAC_OFFSET + 6] = 0x00;

        std::memcpy(data + ADDRTYPE_OFFSET, &isStatic, sizeof(isStatic));
        ...
        inet_pton(addressFamily, ipAddress.c_str(), (data + IPADDR_OFFSET));

        uint8_t prefixOffset = IPADDR_OFFSET + addrSize;

        std::memcpy(data + prefixOffset, &prefix, sizeof(prefix));

        uint8_t gatewayOffset = prefixOffset + sizeof(decltype(prefix));

        inet_pton(addressFamily, gateway.c_str(), (data + gatewayOffset));
    }
    ...
    std::memcpy(data, net_conf_initial_bytes, sizeof(net_conf_initial_bytes));

    std::memcpy(data + ADDR_SIZE_OFFSET, &addrSize, sizeof(addrSize));
...
/** @brief Get Chassis  system boot option
 ...
 *                          blockSelector
 *  @return returns ipmi completion code
 *  @param  version       -  parameter version
 *  @param  parm         -  parameter vaild/invaild indicator
 *  @param  paramDataBootOption - boot option parameter selector
 *  @param  paramDataEnable     - parameter data
 *
 */
ipmi::RspType<uint8_t, // parameter version
              uint8_t, // boot option parameter selector
              uint8_t, // bootoption parameters
              uint8_t  // parameter data
              >
    ipmiChassisGetSysBootOptions(uint7_t bootOptionParameter, bool reserved1,

                                 uint8_t setSelector, uint8_t blockSelector)
{
    constexpr uint8_t bootOptionMask = 2;
    constexpr uint8_t parameterBootFlag = 5;
    constexpr uint8_t bootOptionParam = 0;
    constexpr uint8_t paramData = 1;
    constexpr uint8_t setParmVersion = 0x01;
    ...
    version = setParmVersion;
    parm = parameterBootFlag;
    data[bootOptionParam] = setParmBootFlagsValidOneTime;
        ...
        static_cast<uint8_t>(BootOptionParameter::BOOT_FLAGS))
    {
        ...
        try
        {
            ...
            data[bootOptionParam] = (bootOption << bootOptionMask);

            data[paramData] = oneTimeEnabled ? setParmBootFlagsValidOneTime
                                             : setParmBootFlagsValidPermanent;
            return ipmi::responseSuccess(version, parm, data[bootOptionParam],
                                         data[paramData]);
        }
        catch (InternalFailure& e)
        {
             ...
             static_cast<uint8_t>(BootOptionParameter::OPAL_NETWORK_SETTINGS))
    {
        ...
        auto oneTimeEnabled =
            std::get<settings::boot::OneTimeEnabled>(bootSetting);
        parm = static_cast<uint8_t>(BootOptionParameter::OPAL_NETWORK_SETTINGS);

        int ret = getHostNetworkData(&version, &parm, data);
                                             ...
                                             ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    if (reqptr->parameter == (uint8_t)BootOptionParameter::BOOT_FLAGS)
    {
        ...
        try
        {
            bool permanent = (reqptr->data[0] & setParmBootFlagsPermanent) ==
