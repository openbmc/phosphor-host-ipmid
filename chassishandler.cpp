...
static ipmi::Cc setBootType(ipmi::Context::ptr& ctx, const Type::Types& type)
{
    ...
    {
        // Don't report error on failure as BootType interface
        // may be not present. This interface is not relevant
        // for some Host architectures (for example POWER)
        return ipmi::ccSuccess;
    }
    return ipmi::ccSuccess;
}
        ...
        static_cast<uint8_t>(BootOptionParameter::bootFlags))
    {
        ...
        try
        {
            ...
            if (ec)
            {
                ...
                // may be not present. This interface is not relevant
                // for some Host architectures (for example POWER).
                // Always report bootType as EFI in this case
                auto bootType = Type::Types::EFI;
            }
            else
            {
