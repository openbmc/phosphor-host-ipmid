...
//  Currently mapper doesn't give the readable busname(gives busid) so we can not
...
int getHostNetworkData(get_sys_boot_options_response_t* respptr)
{
    ...
    try
    {
        ...
        // objects,To handle such cases we are interested on that object
        //  which are on interested busname.
        //  Currenlty mapper doesn't give the readable busname(gives busid) so we can not
        //  match with bus name so giving some object specific info as
        //  SETTINGS_MATCH.
