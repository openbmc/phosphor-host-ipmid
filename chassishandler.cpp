...
/** @brief  Implements get chassis capalibilites command
 *  @param
 *   - None
 ...
 *
 */
ipmi::RspType<uint8_t, // chassis capailities flag
    ...
    ipmiGetChassisCap()
{
    try
    {
        ...
        auto chassisCapFlags = ipmi::getDbusProperty(
            bus, chassisCapObject.second, chassisCapObject.first,
            chassisCapIntf, chassisCapFlagsProp);

        // Chassis FRUInfo Devie Address
        auto chassisFRUInfoDevAddr = ipmi::getDbusProperty(
            bus, chassisCapObject.second, chassisCapObject.first,
            chassisCapIntf, chassisFRUDevAddrProp);

        // Chassis SDR Device Address.
        auto chassisSDRDevAddr = ipmi::getDbusProperty(
            bus, chassisCapObject.second, chassisCapObject.first,
            chassisCapIntf, chassisSDRDevAddrProp);

        // Chassis SEL Device Address.
        auto chassisSELDevAddr = ipmi::getDbusProperty(
            bus, chassisCapObject.second, chassisCapObject.first,
            chassisCapIntf, chassisSELDevAddrProp);

        // Chassis System Management Device Address.
        auto chassisSMDevAddr = ipmi::getDbusProperty(
            bus, chassisCapObject.second, chassisCapObject.first,
            chassisCapIntf, chassisSMDevAddrProp);

        // Chassis Bridge Device Address.
        auto chassisBridgeDevAddr = ipmi::getDbusProperty(
            bus, chassisCapObject.second, chassisCapObject.first,
            chassisCapIntf, chassisBridgeDevAddrProp);
        return ipmi::responseSuccess((std::get<uint8_t>(chassisCapFlags)),
                                     (std::get<uint8_t>(chassisFRUInfoDevAddr)),
                                     (std::get<uint8_t>(chassisSDRDevAddr)),
                                     (std::get<uint8_t>(chassisSELDevAddr)),
                                     (std::get<uint8_t>(chassisSMDevAddr)),
                                     (std::get<uint8_t>(chassisBridgeDevAddr)));
    }
