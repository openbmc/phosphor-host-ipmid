...
static std::optional<uint24_t> getBootMboxIANA(ipmi_ret_t& ret)
{
    using namespace chassis::internal;
    using namespace chassis::internal::cache;
...
// not modifying the completion code when, because modification
// is only needed on errors.
static std::optional<MboxVec> getBootMbox(ipmi_ret_t& ret)
{
    using namespace chassis::internal;
    using namespace chassis::internal::cache;
...
// not modifying the completion code when, because modification
// is only needed on errors.
static bool setBootMbox(ipmi_ret_t& ret, MboxVec data)
{
    using namespace chassis::internal;
    using namespace chassis::internal::cache;
        ...
        *data_len = 0; // Assume an error and no data
        ...
        try
        {
            ...
            if (0 == reqBlock)
            {
                ...
                std::optional<uint24_t> IANAEnterprise = getBootMboxIANA(rc);
                if (!IANAEnterprise)
                {
            ...
            std::optional<MboxVec> dataVec = getBootMbox(rc);
            if (!dataVec)
            {
                return IPMI_CC_INVALID;
            }
    ...
    *data_len = 0;
             ...
             static_cast<uint8_t>(BootOptionParameter::BOOT_INITIATOR_MBOX))
    {
        ...
        try
        {
            ...
            if (setBootMbox(rc, *dataVec) && (IPMI_CC_OK != rc))
            {
                return rc;
            }
        }
