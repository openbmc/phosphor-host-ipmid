    ...
    try
    {
        do
        {
            ...
            if (std::equal(msgPayloadStartingPos + cookieOffset,
                           msgPayloadStartingPos + cookieOffset + sizeCookie,
            ...
            std::copy(msgPayloadStartingPos + addrTypeOffset,
                      msgPayloadStartingPos + addrTypeOffset + sizeof(decltype(addrOrigin)),
                      std::ostream_iterator<int>(result, ""));
            ...
            std::copy(msgPayloadStartingPos + addrSizeOffset,
                      (msgPayloadStartingPos + addrSizeOffset + sizeof(decltype(addrSize))),
                      &addrSize);

            uint8_t prefixOffset = ipAddrOffset + addrSize;
            std::copy(msgPayloadStartingPos + prefixOffset,
                      (msgPayloadStartingPos + prefixOffset + sizeof(decltype(prefix))),
                      &prefix);
            ...
            ipAddress = getAddrStr(family, msgPayloadStartingPos, ipAddrOffset, addrSize);

            gateway = getAddrStr(family, msgPayloadStartingPos, gatewayOffset, addrSize);
