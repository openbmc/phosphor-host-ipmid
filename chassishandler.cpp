...
{
    constexpr char const* nmiObjPath = "/xyz/openbmc_project/control/host0/nmi";
    constexpr char const* nmiIntfName = "xyz.openbmc_project.Control.Host.NMI";

    std::string service;
    boost::system::error_code ec = ipmi::getService(ctx, nmiIntfName,
                                                    nmiObjPath, service);
    if (ec)
    {
    ...
    auto nmiCall = chassis::internal::dbus.new_method_call(service.c_str(),
                                                           nmiObjPath,
                                                           nmiIntfName, "NMI");
    try
    {
        chassis::internal::dbus.call(nmiCall);
        return 0;
    }
    catch (const std::exception& e)
    {
        log<level::ERR>("NMI call failed", entry("ERROR=%s", e.what()));
        elog<InternalFailure>();
        return -1;
    }
}
