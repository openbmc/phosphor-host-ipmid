...
int getHostNetworkData(ipmi::Context::ptr& ctx, ipmi::message::Payload& payload)
{
    ...
    auto ipAddress = ipmi::propertyValueOrDefault<std::string>(
        properties, NetworkIP::property_names::address);

    auto gateway = ipmi::propertyValueOrDefault<std::string>(
        properties, NetworkIP::property_names::gateway);

    auto prefix = ipmi::propertyValueOrDefault<uint8_t>(
        properties, NetworkIP::property_names::prefix_length);

    static const std::string staticIP{
        "xyz.openbmc_project.Network.IP.AddressOrigin.Static"};
    uint8_t isStatic =
        (ipmi::propertyValueOrDefault<std::string>(
             properties, NetworkIP::property_names::origin, staticIP) ==
         staticIP)
            ? 1
            : 0;
    ...
    uint8_t addressFamily =
        (ipmi::propertyValueOrDefault<std::string>(
             properties, NetworkIP::property_names::type, protocolIPv4) ==
         protocolIPv4)
            ? AF_INET
            : AF_INET6;
