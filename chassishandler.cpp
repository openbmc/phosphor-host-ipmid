...
// length and gateway.
static constexpr uint8_t netConfValidByte = 0x80; // purpose unclear
...
int getHostNetworkData(ipmi::Context::ptr& ctx, ipmi::message::Payload& payload)
{
    ...
    std::array<uint8_t, macAddrLen> macBytes{};
    {
        ...
        for (auto& byte : macBytes)
        {
            if (!std::getline(ss, token, ':') ||
                !ipmi::tryParse(token, byte, 16))
            {
                break;
            }
        }
...
/** @brief unpack a fixed number of bytes from an IPMI payload.
 *  @param[in] data - the payload to read from.
 *  @param[out] out - pre-sized buffer to fill; exactly out.size() bytes are
 *                    consumed from the payload.
 *  @returns 0 on success, non-zero if the payload has too few bytes remaining.
 */
static int unpackBytes(ipmi::message::Payload& data, std::vector<uint8_t>& out)
{
    for (auto& byte : out)
    {
        if (data.unpack(byte) != 0)
        {
            return -1;
        }
    }
                            ...
                            ipmi::message::Payload& data)
{
    ...
    setNetProp(NetworkIP::property_names::address, std::string(ipAddress));
    setNetProp(NetworkIP::property_names::prefix_length, prefix);
    setNetProp(NetworkIP::property_names::origin, addressOrigin);
    setNetProp(NetworkIP::property_names::gateway, std::string(gateway));
    setNetProp(NetworkIP::property_names::type,
