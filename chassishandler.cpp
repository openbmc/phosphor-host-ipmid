...
constexpr size_t sizeBootOption = static_cast<uint8_t>(
                                 ...
                                 uint8_t setSelector, uint8_t blockSelector)
{
    ...
    response.pack(version, reserved);
    size_t dataLen;
    ...
    {
        dataLen = static_cast<size_t>(BootOptionResponseSize::BOOT_FLAGS);
        ...
        try
        {
            ...
            response.pack(bootOptionParameter,
                          static_cast<uint1_t>(parmIndicator));
            response.pack(data);
    ...
    {
        dataLen =
            static_cast<size_t>(BootOptionResponseSize::OPAL_NETWORK_SETTINGS);
        std::vector<uint8_t> data(dataLen, 0);
        ...
        if (ret < 0)
        {
            log<level::ERR>("getHostNetworkData failed for GetSysBootOptions.");
            return ipmi::responseUnspecifiedError();
        }
        else
        {
