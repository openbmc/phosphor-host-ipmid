...
constexpr size_t SIZE_BOOT_OPTION =
        (uint8_t) BootOptionResponseSize::OPAL_NETWORK_SETTINGS; //Maximum size of the boot option parametrs
...
//PetiBoot-Specific
static constexpr uint8_t net_conf_initial_bytes[] =
                       ...
                       const std::string& conf_str)
{
    ...
    constexpr
    auto COMMA_DELIMITER = ",";
    constexpr
    auto EQUAL_DELIMITER = "=";
    ...
    while (commaDelimtrPos < conf_str.length())
    {
        ...
        if (0
                == conf_str.compare(commaDelimtrPrevPos,
...
int getHostNetworkData(get_sys_boot_options_response_t* respptr)
{
    ...
    {
        fprintf(stderr,
                "Dbus get property(boot_flags) failed\
...
int setHostNetworkData(set_sys_boot_options_t * reqptr)
{
    ...
    { 0 };
    char dhcp[SIZE_PREFIX] =
    { 0 };
    char prefix[SIZE_PREFIX] =
    { 0 };
    int rc = 0;
    ...
    do
    {
        ...
        if (memcmp(&(reqptr->data[COOKIE_OFFSET]),
                   (net_conf_initial_bytes + COOKIE_OFFSET), SIZE_COOKIE) != 0)
        {
            //cookie == 0
            if (memcmp(&(reqptr->data[COOKIE_OFFSET]), &zeroCookie, SIZE_COOKIE)
                    == 0)
            {
                rc = 0;
                break;
        ...
        if (memcmp(&(reqptr->data[VERSION_OFFSET]),
                   (net_conf_initial_bytes + VERSION_OFFSET), SIZE_VERSION)
                != 0)
        {
        ...
        snprintf(mac, SIZE_MAC, MAC_ADDRESS_FORMAT, reqptr->data[MAC_OFFSET],
                 reqptr->data[MAC_OFFSET + 1], reqptr->data[MAC_OFFSET + 2],
                 reqptr->data[MAC_OFFSET + 3], reqptr->data[MAC_OFFSET + 4],
                 reqptr->data[MAC_OFFSET + 5]);

        snprintf(dhcp, SIZE_PREFIX, ADDR_TYPE_FORMAT,
                 reqptr->data[ADDRTYPE_OFFSET]);
        ...
        snprintf(prefix, SIZE_PREFIX, PREFIX_FORMAT,
                 reqptr->data[PREFIX_OFFSET]);
    ...
    if (!rc)
    {
        ...
        host_network_config += "ipaddress=" + std::string(ipAddress)
                + ",prefix=" + std::string(prefix) + ",gateway="
                + std::string(gateway) + ",mac=" + std::string(mac)
                + ",addr_type=" + std::string(dhcp);
        ...
        {
            fprintf(stderr,
                    "Dbus set property(network_config)\
...
int ipmi_chassis_power_control(const char *method)
{
    ...
    finish: sd_bus_error_free(&bus_error);
                                   ...
                                   ipmi_context_t context)
{
    ...
    *data_len = 4;
    ...
    {
        fprintf(stderr,
                "Dbus get property(power_policy) failed for get_sys_boot_options.\n");
        rc = IPMI_CC_UNSPECIFIED_ERROR;
    }
    else
    {
                                             ...
                                             ipmi_context_t context)
{
            ...
            == static_cast<uint8_t>(BootOptionParameter::BOOT_FLAGS))
    {
        ...
        *data_len = static_cast<uint8_t>(BootOptionResponseSize::BOOT_FLAGS);
        ...
        {
            fprintf(stderr,
                    "Dbus get property(boot_flags) failed for get_sys_boot_options.\n");
            rc = IPMI_CC_UNSPECIFIED_ERROR;
        ...
        {
            fprintf(stderr,
                    "Dbus get property(boot_policy) failed for get_sys_boot_options.\n");
            rc = IPMI_CC_UNSPECIFIED_ERROR;
        ...
        else
        {
            ...
            resp->data[0] =
                    (strncmp(p, "ONETIME", strlen("ONETIME")) == 0) ? SET_PARM_BOOT_FLAGS_VALID_ONE_TIME :
    ...
    *data_len = 0;
    ...
    if (reqptr->parameter == (uint8_t) BootOptionParameter::BOOT_FLAGS)
    {
        ...
        else
        {
            ...
            {
                fprintf(stderr,
                        "Dbus set property(boot_flags) failed for set_sys_boot_options.\n");
                rc = IPMI_CC_UNSPECIFIED_ERROR;
            }
        }
        ...
        s = (char *) (
                ((reqptr->data[0] & SET_PARM_BOOT_FLAGS_PERMANENT)
                        == SET_PARM_BOOT_FLAGS_PERMANENT) ? "PERMANENT" :
                        "ONETIME");
        ...
        {
            fprintf(stderr,
                    "Dbus set property(boot_policy) failed for set_sys_boot_options.\n");
            rc = IPMI_CC_UNSPECIFIED_ERROR;
        }
