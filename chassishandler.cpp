...
bool getACFailStatus()
{
    ...
    catch (const std::exception& e)
    {
        lg2::error(
            ...
            "PATH", powerControlObj, "INTERFACE", powerControlIntf, "ERROR",
            e.what());
    }
    return acFail;
}
...
static ipmi::Cc getBootType(ipmi::Context::ptr& ctx, Type::Types& type)
{
    ...
    if (!ec)
    {
        ...
        if (ec)
        {
            lg2::error("Error in BootType Get: {ERROR}",
                       "ERROR", ec.message();
            return ipmi::ccUnspecifiedError;
        }
