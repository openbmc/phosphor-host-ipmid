...
using logging = phosphor::logging;
                               ...
                               bool isNonVolatile)
{
    ...
    if (true == isNonVolatile)
    {
        try
        {
            ...
            if (!outFile.good())
            {
                logging::log<logging::level::ERR>("can not create JSON file");
                return;
            }
        ...
        catch (std::ios_base::failure& e)
        {
            logging::log<logging::level::ERR>(
                "Exception", logging::entry("EXCEPTION=%s", e.what()));
        }
    }
...
int SysInfoParamStore::restore(uint8_t paramSelector)
{
    ...
    try
    {
        ...
        if (!infile.good())
        {
            logging::log<logging::level::ERR>(
                "file does not exist",
                logging::entry("FILE=%s", filename.c_str()));
            return -1;
        }
    ...
    {
        logging::log<logging::level::ERR>("Json Exception caught.",
                                          logging::entry("MSG:%s", e.what()));
        return -1;
    }
