...
GetSensorResponse readingData(const Info& sensorInfo)
{
    ...
    Value propValue, invalidValue;
    propValue = ipmi::getDbusProperty(
        bus, service, sensorInfo.sensorPath,
        sensorInfo.propertyInterfaces.begin()->first,
        sensorInfo.propertyInterfaces.begin()->second.begin()->first);

    if (invalidValue == propValue)
    {
        // This may happen when a cached dbus sensor object disappears
        // due to the removal/addition of a sensor.
        getSensorServiceCache().invalidateAll();
        log<level::ERR>("Invalidated cache and re-reading cache.");
        printf("Invalidated cache and re-reading cache.\n");

        // Attempt to re-read the sensor
        service = getSensorServiceCache().getService(bus, sensorInfo);
        propValue = ipmi::getDbusProperty(
            bus, service, sensorInfo.sensorPath,
            sensorInfo.propertyInterfaces.begin()->first,
            sensorInfo.propertyInterfaces.begin()->second.begin()->first);
