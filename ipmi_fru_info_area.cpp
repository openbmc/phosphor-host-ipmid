...
FruAreaData buildFruAreaData(const FruInventoryData& invData)
{
    ...
    auto chassisIt = invData.find(chassis);
    if(chassisIt != invData.end())
    {
        auto temp = buildChassisInfoArea(chassisIt->second);
        chassisArea.insert(chassisArea.end(), temp.begin(), temp.end());
    }
    buildCommonHeaderSection(chassisArea.size(), curDataOffset, combFruArea);

    //4th byte is offset to board data
    FruAreaData boardArea;
    auto boardIt = invData.find(board);
    if(boardIt != invData.end())
    {
        auto temp = buildBoardInfoArea(boardIt->second);
        boardArea.insert(boardArea.end(), temp.begin(), temp.end());
    }

    //5th byte is offset to product data
    FruAreaData prodArea;
    auto prodIt = invData.find(product);
    if(prodIt != invData.end())
    {
        auto temp = buildProductInfoArea(prodIt->second);
        prodArea.insert(prodArea.end(), temp.begin(), temp.end());
    }
