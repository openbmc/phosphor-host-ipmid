    ...
    ipmiAppGetSessionInfo(uint8_t sessionIndex, ipmi::message::Payload& payload)
{
    ...
    try
    {
        ...
        for (auto& objectTreeItr : objectTree)
        {
                ...
                (static_cast<uint8_t>(session::State::active)))
            {
                ...
                privLevel = std::get<uint8_t>(sessionProps["CurrentPrivilege"]);
                channelNumber = std::get<uint8_t>(sessionProps["ChannelNum"]);
                remoteIpAddr = std::get<uint32_t>(sessionProps["RemoteIPAddr"]);
                remotePort = std::get<uint16_t>(sessionProps["RemotePort"]);
            }
            else
            {
                sessionHandle = 0;
                return ipmi::responseSuccess(sessionHandle, totalSessionCount,
                                             activeSessionCount, std::nullopt);
            }
        }
    }
    ...
    {
        return ipmi::responseSuccess(
            sessionHandle, totalSessionCount, activeSessionCount,
