...
static constexpr auto BMC = "BMC";
static constexpr auto Match = "";
static constexpr auto Priority = "Priority";
static constexpr auto Purpose = "Purpose";
 ...
 * The RedundancyPriority interface provides the relationship between
 * two (or more) software versions activated for a single managed element and
 * the property "Priority" shows which version is the primary (with low value)
 * and which are available for redundancy.
 *
...
std::string getActiveSoftwareVersionInfo()
{
    ...
    try
    {
        auto softObject = ipmi::getDbusObject(bus,
                                              redundancyIntf,
                                              softwareRoot,
                                              Match);
        ...
        auto objValueTree = ipmi::getManagedObjects(bus,
                            service,
                            softwareRoot);

        sdbusplus::message::object_path objPath{};
        // Map of redundancy priority vs software versionInf of all active s/w
        // objects of BMC.
        std::map<uint8_t, std::string> versionMap;
        for (const auto& objIter : objValueTree)
        {
            auto priority = objValueTree.at(objIter.first).at(std::string(
                                redundancyIntf)).at(std::string(
                                        Priority)).get<uint8_t>();
            auto purpose =  objValueTree.at(objIter.first).at(std::string(
                                versionIntf)).at(std::string(
                                        Purpose)).get<std::string>();
            version = objValueTree.at(objIter.first).at(std::string(
                          versionIntf)).at(std::string(
                                               Version)).get<std::string>();
            auto actvation = objValueTree.at(objIter.first).at(std::string(
                                 activationIntf)).at(std::string(
                                             Activation)).get<std::string>();

            if ((purpose.find(BMC) != std::string::npos) &&
                (actvation.find(ACTIVE) != std::string::npos))
            {
                versionMap[priority] = version;
            }
        }
        // Retun version info of first s/w object from list all BMC active s/w
        // objects those are sorted based on redundancy priority.
        version = versionMap.begin()->second;
        versionMap.clear();
        return version;
    }
    ...
    {
        log<level::ERR>("Could not found software version info for BMC");
