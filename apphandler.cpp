...
ipmi::RspType<
    ...
    ipmiAppGetSystemInfo(uint1_t getRevision, uint7_t reserved,
    ...
    std::optional<std::array<uint8_t, configParameterLength>> dataN)
{
    if (paramSelector == 0)
    {
        ...
        if ((data1 != setComplete) && (data1 != setInProgress))
        {
            return ipmi::responseInvalidFieldRequest();
        }
    ...
    if (!data1 || !dataN || (dataN.value().size() != configParameterLength))
    {
        return ipmi::responseInvalidFieldRequest();
    }
