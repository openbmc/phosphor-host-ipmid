...
std::string getActiveSoftwareVersionInfo(ipmi::Context::ptr ctx)
{
    ...
    for (auto& softObject : objectTree)
    {
        ...
        std::string service;
        boost::system::error_code ec =
            ipmi::getService(ctx, redundancyIntf, softObject.first, service);
        if (!ec)
        {
                                    ...
                                    const bool fallbackAvailability)
{
    ...
    boost::system::error_code ec =
        ipmi::getDbusObject(ctx, bmc_state_interface, "/", {}, bmcObject);
    if (ec)
    {
    ...
    ec = ipmi::getDbusProperty(ctx, bmcObject.second, bmcObject.first,
                               bmc_state_interface, bmc_state_property, state);
    if (ec)
    {
        return fallbackAvailability;
    }

    return BMC::convertBMCStateFromString(state) == BMC::BMCState::Ready;
}
