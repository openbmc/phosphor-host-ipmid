...
static const char  *app_obj   =  "/org/openbmc/NetworkManager/Interface";
static const char  *app_ifc   =  "org.openbmc.NetworkManager";
static const char *app_nwinterface = "eth0";
...
bool check_empty( const char *data, char Len )
{
    while(Len--)
    {
                             ...
                             ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    if( !strcmp(new_ipaddr, "") || !check_empty( (const char *)new_ipaddr, (char)INET_ADDRSTRLEN) )
    {
        memcpy(new_ipaddr, ipaddr, INET_ADDRSTRLEN);
    }
    if( !strcmp(new_netmask, "") || !check_empty( (const char *)new_netmask, (char)INET_ADDRSTRLEN) )
    {
    ...
    if( !strcmp(new_gateway, "") || !check_empty( (const char *)new_gateway, (char)INET_ADDRSTRLEN) )
    {
