                                  ...
                                  ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    auto objValueTree = ipmi::getManagedObjects(
        bus, "xyz.openbmc_project.netipmid", session::SESSION_MANAGER_ROOT);
    for (const auto& objIter : objValueTree)
    {
        try
        {
            ...
            if (req->sessionID == 0)
            {
                    ...
                    req->sessionHandle != 0)
                {
                    ...
                    ipmi::setDbusProperty(
                        ...
                        static_cast<uint8_t>(
                            session::SessionState::TEAR_DOWN_IN_PROGRESS));
                    return IPMI_CC_OK;
                }
            }
            else
                                     ...
                                     ipmi_context_t context)
{
    ...
    auto objValueTree = ipmi::getManagedObjects(
        bus, "xyz.openbmc_project.netipmid", session::SESSION_MANAGER_ROOT);
    resp.totalSessionCount = session::MAX_SESSION_COUNT;
    resp.ipmiVer = session::ipmi20VerSession;

    switch (req->sessionIndex)
    {
        case session::searchSessionByHandle:

            if (req->sessionHandle >= session::MAX_SESSION_COUNT)
            {
                *data_len = 0;
                return IPMI_CC_INVALID_FIELD_REQUEST;
            }

            req->sessionIndex = req->sessionHandle;
            break;
        ...
        case session::searchSessionByID:
            ...
            try
            {
                ...
                sstream << std::hex << req->sessionID;
                std::string reqsessionIDStr = sstream.str();
                auto objPath = std::string(session::SESSION_MANAGER_ROOT) +
                               "/" + reqsessionIDStr;
                // Read the session properties
                auto sessionProps = ipmi::getAllDbusProperties(
                    bus, session::service, objPath, session::sessionIntf);
        ...
        default:
            ...
            if (req->sessionIndex == 0 ||
                req->sessionIndex >= session::MAX_SESSION_COUNT)
            {
                *data_len = 0;
                return IPMI_CC_INVALID_FIELD_REQUEST;
            }
