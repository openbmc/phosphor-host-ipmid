...
constexpr auto protocolType = "xyz.openbmc_project.Network.IP.Protocol.IPv4";
                                   ...
                                   ipmi_context_t context)
{
    ...
    try
    {
        ...
        // but it always expected that system would be having
        // ethernet interface and system config object.
        try
        {
            ...
            auto properties = ipmi::getAllDbusProperties(ipObjectInfo.second,
                                                         ipObjectInfo.first,
                                                         ipmi::IP_INTERFACE);
        ...
        catch (InternalFailure& e)
        {
            log<level::INFO>("Failed to get IP object which matches",
                             entry("INTERFACE=%s", ipmi::IP_INTERFACE),
                             entry("MATCH=%s", ipmi::IP_TYPE));
        }
        auto systemObject = ipmi::getDbusObject(ipmi::SYSTEMCONFIG_INTERFACE,
                                                ipmi::NETWORK_ROOT);

        auto networkInterfaceObject = ipmi::getDbusObject(
                                          ipmi::ETHERNET_INTERFACE,
                                          ipmi::NETWORK_ROOT,
                                          ipmi::INTERFACE);
        ...
        {
            channelConfig.ipaddr = ipaddress;
        }

        if (!channelConfig.netmask.empty())
        {
            //convert mask into prefix
            prefix = ipmi::toPrefix(AF_INET, channelConfig.netmask);
        }
        ...
        ipmi::deleteAllDbusObjects(ipmi::NETWORK_ROOT, ipmi::VLAN_INTERFACE);
        ipmi::deleteAllDbusObjects(ipmi::NETWORK_ROOT, ipmi::IP_INTERFACE,
                                  ipmi::IP_TYPE);
        if (vlanID)
        {
        ...
        ipmi::createIP(networkInterfaceObject.second,
                       ...
                       protocolType, channelConfig.ipaddr, prefix);

        ipmi::setDbusProperty(systemObject.second, systemObject.first,
                              ipmi::SYSTEMCONFIG_INTERFACE, "DefaultGateway",
