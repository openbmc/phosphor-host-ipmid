...
std::string getActiveSoftwareVersionInfo(ipmi::Context::ptr ctx)
{
    ...
    {
        lg2::error(
            "Failed to fetch redundancy object from dbus, interface: {INTERFACE},  error: {ERROR}",
    ...
    -> ipmi::RspType<std::array<uint8_t, 16>>
{
    ...
    {
        lg2::error(
            "Failed to locate System UUID object, interface: {INTERFACE}, error: {ERROR}",
    ...
    {
        lg2::error(
            "Failed to read System UUID property, interface: {INTERFACE}, property: {PROPERTY}, error: {ERROR}",
            "INTERFACE", uuidInterface, "PROPERTY", uuidProperty, "ERROR",
            ec.message());
        return ipmi::responseUnspecifiedError();
    }
    ...
    {
        lg2::error(
            "Failed in parsing BMC UUID property, interface: {INTERFACE}, property: {PROPERTY}, value: {VALUE}, error: {ERROR}",
            "INTERFACE", uuidInterface, "PROPERTY", uuidProperty, "VALUE",
            rfc4122Uuid, "ERROR", e);
        return ipmi::responseUnspecifiedError();
    }
    return ipmi::responseSuccess(uuid);
                        ...
                        const std::string& service, const std::string& obj)
{
    ...
    {
        lg2::error(
            "Failed in getting session state property, service: {SERVICE}, object path: {OBJECT_PATH}, interface: {INTERFACE}, error: {ERROR}",
                        ...
                        const std::string& objPath, uint8_t& sessionState)
{
    ...
    {
        lg2::error(
            "Failed to fetch state property, service: {SERVICE}, object path: {OBJECTPATH}, interface: {INTERFACE}, error: {ERROR}",
...
static bool populateI2CControllerWRAllowlist()
{
    ...
    {
        lg2::error(
            "Corrupted i2c allow list config file, file name: {FILE_NAME}, error: {ERROR}",
    ...
    try
    {
        ...
        for (const auto& it : filters.items())
        {
            ...
            {
                lg2::error(
                    "Corrupted I2C controller write read allowlist config file, file name: {FILE_NAME}",
                    "FILE_NAME", i2cControllerWRAllowlistFile);
                return false;
            }
    ...
    {
        lg2::error(
            "I2C controller write read allowlist unexpected exception: {ERROR}",
            "ERROR", e);
        return false;
    }
    return true;
                            ...
                            std::vector<uint8_t> writeData)
{
    ...
    {
        lg2::error(
            "Controller write read request blocked!, bus: {BUS}, addr: {ADDR}",
