                            ...
                            uint8_t* chunk)
{
    ...
    try
    {
        if (chunkIndex == 0)
        {
            ...
            std::copy_n(info.stringDataN.begin(), smallChunkSize, chunk);
            return smallChunkSize;
        }
        else
        {
            // Output must have 16 byte capacity.
            std::copy_n(info.stringDataN.begin() + chunkIndex * chunkSize,
                        chunkSize, chunk);
            return chunkSize;
        }
    }
                               ...
                               uint8_t chunkLen)
{
    ...
    // lookup
    std::optional<IpmiSysInfo> param = sysInfoParamStore->lookup(paraSelector);
    ...
    if (setSelector == 0) // chunk0
    {
        ...
        // cleanup
        std::fill_n(info.stringDataN.begin(), maxBytesPerParameter, 0);
    ...
    std::copy_n((chunk + headerBytes), validLen,
                (info.stringDataN.begin() + setSelector * chunkSize));
                                    ...
                                    ipmi_context_t context)
{
    ...
    *dataLen = 0; // default to 0.
    if (paramRequested == 0)
    {
        ...
        if ((transferStatus != setComplete) && (setInProgress == chunk[0]))
        {
            return IPMI_CC_SET_IN_PROGRESS_ACTIVE;
        }
        // commit write is not supported
        if ((setComplete != chunk[0]) && (setComplete != chunk[0]))
        {
            return IPMI_CC_INVALID_FIELD_REQUEST;
        }
                                    ...
                                    ipmi_context_t context)
{
    ...
    *dataLen = 0; // default to 0.
    ...
    info = sysInfoParamStore->lookup(paramRequested);
    ...
    rc = packGetSysInfoResp(info.value(), reqData[2], &resp);
    if (rc == -EINVAL)
    {
        return IPMI_CC_RESPONSE_ERROR;
    }
