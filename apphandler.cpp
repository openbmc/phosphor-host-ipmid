...
bool check_empty( char *data, size_t len )
{
    return &data[len] != std::find_if(data, &data[len], [](auto x){ return x != 0; });
}
                             ...
                             ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    printf("N/W data from Cache: %s:%s:%s\n",
            ...
            channel_config.new_netmask,
            channel_config.new_gateway);

    if( !strcmp(channel_config.new_ipaddr, "") ||
        !check_empty( (char *)channel_config.new_ipaddr,
                      ( char)INET_ADDRSTRLEN) ) {
        memcpy(channel_config.new_ipaddr, ipaddr, INET_ADDRSTRLEN);
    }
    if( !strcmp(channel_config.new_netmask, "") ||
        !check_empty( (char *)channel_config.new_netmask,
                      (char)INET_ADDRSTRLEN) ) {
        mask = htonl(mask<<(32-prefixlen));

        uint8_t *p = (uint8_t *)&mask;

        snprintf(channel_config.new_netmask, INET_ADDRSTRLEN, "%d.%d.%d.%d",
            *p, *(p+1), *(p+2), *(p+3));
    }
    if( !strcmp(channel_config.new_gateway, "") ||
        !check_empty( (char *)channel_config.new_gateway,
                      (char)INET_ADDRSTRLEN) )
    {
        memcpy(channel_config.new_gateway, gateway, INET_ADDRSTRLEN);
    }
    ...
    if(r < 0) {
        fprintf(stderr, "Failed to set network data %s:%s:%s %s\n",
                channel_config.new_ipaddr, channel_config.new_netmask,
                channel_config.new_gateway, error.message);
        rc = IPMI_CC_UNSPECIFIED_ERROR;
    }
    memset(channel_config.new_ipaddr, 0, INET_ADDRSTRLEN);
    memset(channel_config.new_netmask, 0, INET_ADDRSTRLEN);
    memset(channel_config.new_gateway, 0, INET_ADDRSTRLEN);
