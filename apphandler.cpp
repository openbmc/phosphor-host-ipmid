...
constexpr auto devidManufidSize = 3;
constexpr auto devidProdidSize = 2;
                             ...
                             ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    *data_len = sizeof(dev_id);
    ...
    memcpy(&dev_id.manuf_id, &devIdInfo.manufId, devidManufidSize);

    // Witherspoon's product ID is hardcoded to 4F42(ASCII 'OB').
    memcpy(&dev_id.prod_id, &devIdInfo.productId, devidProdidSize);
