...
static int loadI2CWhiteList()
{
    ...
    try
    {
        ...
        rwWhiteList.resize(filters.size());

        for (const auto& it : filters)
        {
            nlohmann::json filter = it;
            if (filter.is_null())
            {
            ...
            rwWhiteList[i].slaveAddr = std::stoul(
            ...
            log<level::DEBUG>("IPMI I2C whitelist ", entry("INDEX=%d", i),
                              entry("BUS=%d", rwWhiteList[i].busId),
                              entry("ADDR=0x%x", rwWhiteList[i].slaveAddr),
            ...
            while (std::getline(iss, token, ' '))
            {
                ...
                rwWhiteList[i].data.emplace_back(
                               ...
                               ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    log<level::DEBUG>("checking list ", entry("SIZE=%d", rwWhiteList.size()));
    // TODO command whitelist checking, add regex support
    for (unsigned int i = 0; i < rwWhiteList.size(); i++)
    {
        if ((busId == rwWhiteList[i].busId) &&
            (reqi2c->slaveAddr == rwWhiteList[i].slaveAddr) &&
            (outBuf == rwWhiteList[i].data))
        {
    ...
    // write message
    i2cmsg[0].addr = reqi2c->slaveAddr;
    i2cmsg[0].flags = 0x00;
    i2cmsg[0].len = writeCount;
    i2cmsg[0].buf = outBuf.data();

    // read message
    i2cmsg[1].addr = reqi2c->slaveAddr;
