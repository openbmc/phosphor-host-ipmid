                                             ...
                                             uint8_t entityId)
{
    ...
    if (match != ipmi::dcmi::validEntityId.end())
    {
        ...
        {
            std::string sensorObjPath = sensor.first;
            auto sensorTypeValue = getSensorTypeFromPath(sensorObjPath);
            ...
            if (entityIdValue == match->first || entityIdValue == match->second)
            {
                ...
                {
                    sensorList.push_back({sensorObjPath, sensorTypeValue,
                    ...
                    uint8_t instanceStart)
{
    ...
    for (const auto& sensor : sensorList)
    {
        if (!entityInstance)
        {
            ...
            if (readResult)
            {
                tempReadingVal.push_back(
                    std::make_tuple(tempVal, signBit, sensor.entityInstance));

                if (tempReadingVal.size() >= ipmi::dcmi::maxRecords)
                {
                    break;
                }
            }
