    ...
    ipmiSenGetSensorReading(ipmi::Context::ptr ctx, uint8_t sensnum)
{
    ...
    auto operationalObject =
        sensorMap.find(sensor::operationalStatusInterface);
    if (operationalObject != sensorMap.end())
    {
        ...
        if (const bool* functionalPtr = std::get_if<bool>(&(findFunctional->second)))
        {
            functional = *functionalPtr;
        }
        else
        {
            lg2::error("OperationalStatus property 'Functional' is not a bool type");
            return ipmi::responseResponseError();
        }
        uint8_t operation =
        ...
        uint8_t discreteState = functional ? 0x01  // present, healthy
                                           : 0x03; // present, failed
        return ipmi::responseSuccess(0, operation, discreteState, std::nullopt);
    }
...
bool constructDiscreteSdr(
    ...
    const std::unordered_set<std::string>& ipmiDecoratorPaths,
    uint16_t sensorNum, uint16_t recordID, const std::string& service,
    const std::string& path, uint8_t sensorType,
    get_sdr::SensorDataEventRecord& record)
{
    constructEventSdrHeaderKey(sensorNum, recordID, record);
    DbusInterfaceMap sensorMap;
    ...
    uint8_t readBytes = std::numeric_limits<uint8_t>::max())
{
                  ...
                  sensor::operationalStatusInterface) != interfaces.end())
    {
        ...
        else if (!constructDiscreteSdr(ctx, ipmiDecoratorPaths, sensorNum,
                                       recordID, connection, path,
                                       sensor::sensorTypePowerSupply, record))
        {
            return GENERAL_ERROR;
        }
        recordData.insert(recordData.end(), reinterpret_cast<uint8_t*>(&record),
