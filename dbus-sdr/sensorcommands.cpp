    ...
    ipmiSenGetSensorReading(ipmi::Context::ptr ctx, uint8_t sensnum)
{
    ...
    auto operationalObject = sensorMap.find(sensor::operationalStatusInterface);
    if (operationalObject != sensorMap.end())
    {
        ...
        if (const bool* functionalPtr =
                std::get_if<bool>(&(findFunctional->second)))
        {
        ...
        {
            lg2::error(
                "OperationalStatus property 'Functional' is not a bool type");
            return ipmi::responseResponseError();
        }
        uint8_t operation =
