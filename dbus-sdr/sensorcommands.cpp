                         ...
                         std::string_view path, std::bitset<16>& assertions)
{
    ...
    std::cerr << "path is " << path_str << std::endl;
    ...
    {
        std::cerr << "not index error" << std::endl;
        // Not Indexed
        readProperty(path, value);
        assertions |= value;
        return;
    }
    std::cerr << "Start is " << start << " End is " << end << std::endl;
    for (int i = start; i <= end; ++i)
    {
        auto sdrPath = defaultObjPath + std::to_string(i);
        std::cerr << "sdr path is " << sdrPath << std::endl;
        if (readProperty(sdrPath, value))
        {
            assertions |= value << (start == 0 ? i : (i % start));
        }
    }
    std::cerr << "assertion is " << assertions << std::endl;
}
