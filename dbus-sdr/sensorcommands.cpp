    ...
    uint8_t eventData2, uint8_t eventData3)
{
    ...
    if (sensorObject != sensorMap.end())
    {
        ...
        try
        {
            setDbusProperty(ctx, connection, path, sensor::vrInterface,
                            "Selected", *vrMode);
        }
        // setDbusProperty intended to resolve dbus exception/rc within the
        // function but failed to achieve that. Catch SdBusError in the ipmi
    ...
    ipmiSenGetSensorReading(ipmi::Context::ptr ctx, uint8_t sensnum)
{
    ...
    auto status =
        getSensorConnection(ctx, sensnum, connection, path, interfaces);
    if (status)
    {
        return ipmi::response(status);
    }
    ...
    uint8_t upperNonRecoverable)
{
    ...
    ipmi::Cc status =
        getSensorConnection(ctx, sensorNum, connection, path, interfaces);
    if (status)
    {
        return ipmi::response(status);
    }
