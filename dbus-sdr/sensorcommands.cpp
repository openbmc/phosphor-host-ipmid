...
static constexpr size_t lun0Number = 0;
static constexpr size_t lun1Number = static_cast<uint8_t>(lun1Sensor0 >> 8);
static constexpr size_t lun3Number = static_cast<uint8_t>(lun3Sensor0 >> 8);
                               ...
                               std::optional<uint8_t> count)
{
    ...
    if (count.value_or(0) == getSdrCount)
    {
        ...
        if (ctx->lun == lun1Number)
        {
            recordID += maxSensorsPerLUN;
        }
        else if (ctx->lun == lun3Number)
        {
    ...
    else if (count.value_or(0) == getSensorCount)
    {
        ...
        if ((ctx->lun == lun0Number) && (numSensors > 0))
        {
            sdrCount =
                (numSensors > maxSensorsPerLUN) ? maxSensorsPerLUN : numSensors;
        }
        else if ((ctx->lun == lun1Number) && (numSensors > maxSensorsPerLUN))
        {
