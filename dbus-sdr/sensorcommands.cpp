                         ...
                         int updatePeriod = sensorMapUpdatePeriod)
{
    ...
    // If the path is indexed in the form of [x-y], that means this is a stacked
    // sdr and we need to use ipmi event sensor.
    if (std::regex_match(sensorPath, std::regex("^.*\\[[0-9]+-[0-9]+\\]$")))
    {
        return true;
    }
                         ...
                         std::string_view path, std::bitset<16>& assertions)
{
    ...
    if (std::regex_match(path_str, matches, rgx))
    {
    ...
    ipmiSenGetSensorEventStatus(ipmi::Context::ptr ctx, uint8_t sensorNum)
{
    ...
    // If the path is indexed in the form of [x-y], that means this is a stacked
    // sdr and we need to use ipmi event sensor.
    if (std::regex_match(path, std::regex("^.*\\[[0-9]+-[0-9]+\\]$")))
    {
