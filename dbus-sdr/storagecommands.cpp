    ...
    ipmiStorageGetSELInfo(ipmi::Context::ptr ctx)
{
    ...
    auto [ entries, freeSpace, lastAddTime, lastDelTime, operationSupport ] =
        ctx->bus->yield_method_call<std::tuple<uint16_t, uint16_t, uint32_t,
        uint32_t, uint8_t>>(ctx->yield, ec, selLoggerServiceName, ipmiSELPath,
        ipmiSELAddInterface, "IpmiSelInfo");
    if (ec)
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
