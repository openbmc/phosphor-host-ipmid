...
static bool getSELLogFiles(std::vector<std::filesystem::path>& selLogFiles)
{
    ...
    for (const std::filesystem::directory_entry& dirEnt :
         std::filesystem::directory_iterator(
             dynamic_sensor::ipmi::sel::selLogDir))
    {
        std::string filename = dirEnt.path().filename();
        if (boost::starts_with(filename,
                               dynamic_sensor::ipmi::sel::selLogFilename))
        {
...
using systemEventType = std::tuple<
    ...
    std::array<uint8_t, dynamic_sensor::ipmi::sel::systemEventSize>>; // Event
                                                                      // Data
using oemTsEventType = std::tuple<
    uint32_t, // Timestamp
    std::array<uint8_t, dynamic_sensor::ipmi::sel::oemTsEventSize>>; // Event
                                                                     // Data
using oemEventType =
                           ...
                           uint8_t offset, uint8_t size)
{
    ...
    if (recordType == dynamic_sensor::ipmi::sel::systemEvent)
    {
        ...
        std::array<uint8_t, dynamic_sensor::ipmi::sel::systemEventSize>
            eventData{};
