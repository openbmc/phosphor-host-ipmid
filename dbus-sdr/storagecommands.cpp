...
void writeFruCache(uint8_t devId)
{
    ...
    else if (++wctx.attempts < maxFruWriteAttempts)
    {
        ...
        startWriteTimer(wctx, devId, std::chrono::seconds(writeRetrySeconds));
        return;
    }
    else
    {
                            ...
                            std::chrono::seconds timeout)
{
    if (!wctx.timer)
    {
        // Timer::timeoutHandler() disables the event source after
        // userCallBack() returns, so re-arming from inside the callback is
        // undone. Defer the flush so it runs outside the dispatch.
        wctx.timer = std::make_unique<sdbusplus::Timer>([devId]() {
            post_work([devId]() { writeFruCache(devId); });
        });
    }
    wctx.timer->stop();
    wctx.timer->start(
...
void writeFruIfRunning()
{
    ...
    {
        auto& wctx = it->second;
        if (!wctx.active || !wctx.timer || !wctx.timer->isRunning())
        {
            ++it;
            continue;
        }
        ...
        if (!writeFru(wctx.bus, wctx.addr, wctx.data))
        {
            // Timer stays armed on purpose: writeFruCache() picks this up.
            ++it;
            continue;
        }
