...
namespace dynamic_sensor::ipmi::sel
{
...
static bool getSELLogFiles(std::vector<std::filesystem::path>& selLogFiles)
{
    ...
    for (const std::filesystem::directory_entry& dirEnt :
         std::filesystem::directory_iterator(dynamic_sensor::ipmi::sel::selLogDir))
    {
        std::string filename = dirEnt.path().filename();
        if (boost::starts_with(filename, dynamic_sensor::ipmi::sel::selLogFilename))
        {
            // If we find an ipmi_sel log file, save the path
            selLogFiles.emplace_back(dynamic_sensor::ipmi::sel::selLogDir /
                                     filename);
        }
    }
    ...
    ipmiStorageGetSELInfo()
{
    ...
    uint32_t addTimeStamp = dynamic_sensor::ipmi::sel::getFileTimestamp(
        dynamic_sensor::ipmi::sel::selLogDir / dynamic_sensor::ipmi::sel::selLogFilename);
    uint32_t eraseTimeStamp = dynamic_sensor::ipmi::sel::erase_time::get();
    constexpr uint8_t operationSupport =
        dynamic_sensor::ipmi::sel::selOperationSupport;
...
using systemEventType = std::tuple<
    ...
    std::array<uint8_t, dynamic_sensor::ipmi::sel::systemEventSize>>; // Event Data
using oemTsEventType = std::tuple<
    uint32_t,                                                   // Timestamp
    std::array<uint8_t, dynamic_sensor::ipmi::sel::oemTsEventSize>>; // Event Data
using oemEventType =
    std::array<uint8_t, dynamic_sensor::ipmi::sel::oemEventSize>; // Event Data
                           ...
                           uint8_t offset, uint8_t size)
{
    ...
    if (recordType == dynamic_sensor::ipmi::sel::systemEvent)
    {
        // Get the timestamp
        std::tm timeStruct = {};
        ...
        uint8_t evmRev = dynamic_sensor::ipmi::sel::eventMsgRev;
        ...
        std::array<uint8_t, dynamic_sensor::ipmi::sel::systemEventSize> eventData{};
                                           ...
                                           uint8_t eraseOperation)
{
    ...
    // Save the erase time
    dynamic_sensor::ipmi::sel::erase_time::save();
