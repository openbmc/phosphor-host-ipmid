                    ...
                    const std::vector<std::filesystem::path>& selLogFiles)
{
    ...
    for (auto selFileIter = selLogFiles.rbegin();
         selFileIter != selLogFiles.rend(); selFileIter++)
    {
                                           ...
                                           uint16_t targetID)
{
    ...
    ctx->bus->yield_method_call<void>(ctx->yield, ec, selLoggerServiceName,
    ...
    if (ec)
    {
        if (ec == boost::system::error_condition(
                      EIO, boost::system::generic_category()))
        {
            return ipmi::responseSensorInvalid();
        }
        if (ec == boost::system::error_condition(
