...
ipmi::RspType<uint16_t> ipmiDeleteSELEntry(ipmi::Context::ptr ctx,
                                                uint16_t reservationID, uint16_t targetID)
{
    if (!checkSELReservation(reservationID))
    {
        return ipmi::responseInvalidReservationId();
    ...
    static constexpr uint16_t selInvalidRecID = std::numeric_limits<uint16_t>::max();
    ...
    uint16_t responseID = ctx->bus->yield_method_call<uint16_t>(
                ...
                "xyz.openbmc_project.Logging.IPMI","IpmiSelDelete", targetID);
    ...
    if (ec)
    {
        std::cout << "DeleteSELEntry: Failed to delete System SEL through sdbus" << std::endl;
        return ipmi::responseUnspecifiedError();
    }
