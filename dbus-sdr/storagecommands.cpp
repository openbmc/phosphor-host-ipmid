...
// Release the FRU data buffer but keep the FruWriteCtx entry.
// Do not erase: writeFruCache() runs inside Timer::timeoutHandler(),
// which still touches eventSource after the user callback returns,
// so destroying the Timer here would be a use-after-free.
static void releaseWriteCtx(FruWriteCtx& wctx)
{
    wctx.active = false;
    wctx.data.clear();
...
{
    for (auto& [devId, wctx] : fruWriteCtxMap)
    {
        ...
        lg2::info("writeFruIfRunning: flushing devId={DEVID}, "
                  "bus={BUS}, addr={ADDR}",
                  "DEVID", devId, "BUS", wctx.bus, "ADDR", wctx.addr);
    ...
    std::vector<uint8_t>& dataToWrite)
{
    ...
    if (ctxIt == fruWriteCtxMap.end() || !ctxIt->second.active)
    {
        ...
        // All validation passed, safe to emplace
        auto ctxIt = fruWriteCtxMap.try_emplace(fruDeviceId).first;
    ...
    if (atEnd)
    {
        ...
        countWritten = writeOK ? std::min(fru.size(), static_cast<size_t>(0xFF))
                               : 0;
        releaseWriteCtx(wctx);
