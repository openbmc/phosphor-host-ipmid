        ...
        ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    {
        log<level::ERR>(string("wd_get: ") + e.what());
...
static constexpr unsigned wd_dont_log = 0x1 << 7;
static constexpr unsigned wd_dont_stop = 0x1 << 6;
static constexpr unsigned wd_running = 0x1 << 6;

static constexpr unsigned wd_timeout_action_mask = 0x3;
        ...
        ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    {
        log<level::ERR>(string("wd_set: ") + e.what());
        return IPMI_CC_UNSPECIFIED_ERROR;
    }
    catch (...)
    {
...
IpmiAction wdActionToIpmiAction(Watchdog::Action wd_action)
{
    switch(wd_action)
    {
        ...
        default:
        {
            throw std::domain_error("Watchdog action can't be mapped to IPMI");
        }
    }
}
        ...
        ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    try
    {
        ...
        {
            // We can't actually fail to get the watchdog info because our
            // mapping from watchdog actions to ipmi actions won't work.
            // Just ignore the error and return NONE so the host can reconcile.
            res.timer_action = static_cast<uint8_t>(IpmiAction::None);
        }
        if (wd_prop.enabled)
        {
