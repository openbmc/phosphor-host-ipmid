...
static constexpr uint8_t wdExpirationFlagStartBit = 0x1;
static constexpr uint8_t wdExpirationFlagEndBit = 0x5;
                         ...
                         std::bitset<8> expFlagValue, uint16_t initialCountdown)
{
    ...
    try
    {
        ...
        for (int i = wdExpirationFlagStartBit; i <= wdExpirationFlagEndBit; i++)
        {
            if (expFlagValue.test(i))
            {
                timerUseExpirationFlags.reset(i);
            }
        }
