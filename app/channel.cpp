...
std::pair<std::vector<uint8_t>, std::vector<uint8_t>> getCipherRecords()
{
    ...
    for (const auto& record : data)
    {
        if (record.find(oem) != record.end())
        {
            ...
            cipherRecords.push_back(record.value(oem, 0) >> 8 | integrityTag);
            cipherRecords.push_back(record.value(oem, 0) >> 16 | confTag);
            supportedAlgorithmSet.insert(record.value(oem, 0));
            supportedAlgorithmSet.insert(record.value(oem, 0) >> 8 |
                                         integrityTag);
            supportedAlgorithmSet.insert(record.value(oem, 0) >> 16 | confTag);
        }
        else
        {
