...
class TimerTest : public ::testing::Test
{
    public:
        ...
        // Source of event and the bus to be attached to.
        ...
        TimerTest()
            : rc(sd_event_default(&events)),
              ...
              timer(bus, events, eventSource, timeOutHandler)
        {
            timerExpired = false;
 ...
 *  called post 2 seconds
 */
TEST_F(TimerTest, timeOutAfter2seconds)
{
    using namespace std::chrono;

    auto time = duration_cast<microseconds>(seconds(2));
    EXPECT_GE(timer.startTimer(time.count()), 0);
