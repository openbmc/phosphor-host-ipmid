...
// PEF Configuration parameter selectors (IPMI 2.0 Table 30-6)
...
DbusResult resolveService(ipmi::Context::ptr ctx, std::string& outSvc)
{
    ...
    if (ec)
    {
        lg2::error("PEF: ObjectMapper lookup failed: {ERR}", "ERR",
                   ec.message());
        cache.clear();
                           ...
                           uint8_t& out)
{
    ...
    {
        invalidateService();
        lg2::error("PEF: D-Bus Get property failed: {PROP}: {ERR}", "PROP",
                   prop, "ERR", ec.message());
    ...
    if (parsed == nullptr)
    {
        lg2::error("PEF: D-Bus Get returned wrong type: {PROP}", "PROP", prop);
        return DbusResult::Error;
    }
    out = *parsed;
    return DbusResult::Ok;
                           ...
                           uint8_t val)
{
    ...
    {
        invalidateService();
        lg2::error("PEF: D-Bus Set property failed: {PROP}: {ERR}", "PROP",
                   prop, "ERR", ec.message());
        return classifyEc(ec);
    }
    return DbusResult::Ok;
                         ...
                         const uint8_t* raw)
{
    ...
    ctx->bus->yield_method_call<void>(ctx->yield, ec, svc, pef::pefMgrPath,
                                      pef::pefMgrIntf, pef::methodSetFilter,
                                      num, v);
    if (ec)
    {
        invalidateService();
        lg2::error("PEF: D-Bus SetFilter failed: {ERR}", "ERR", ec.message());
        return classifyEc(ec);
    }
    return DbusResult::Ok;
}
...
DbusResult dbusGetFilter(ipmi::Context::ptr ctx, uint8_t num, uint8_t* raw)
{
    ...
    {
        invalidateService();
        lg2::error("PEF: D-Bus GetFilter failed: {ERR}", "ERR", ec.message());
        return classifyEc(ec);
    }
    if (v.size() != pef::pefEntryBytes)
    {
        lg2::error("PEF: GetFilter returned wrong size");
        return DbusResult::Error;
    }
...
// Cmd 0x12: Set PEF Configuration Parameters
// ---------------------------------------------------------------------------
ipmi::RspType<> ipmiSetPefConfigParams(
    ipmi::Context::ptr ctx, uint8_t paramSelByte, ipmi::message::Payload& req)
{
    ...
    switch (static_cast<PefParam>(paramSel))
    {
        ...
        case PefParam::EventFilterData1:
        {
            ...
            if (req.unpack(filterNum, configByte) != 0 || !req.fullyUnpacked())
            {
                return ipmi::responseReqDataLenInvalid();
            }
            if (filterNum == 0 || filterNum > pef::pefMaxFilters)
            ...
            switch (dbusGetFilter(ctx, filterNum, raw))
            {
                ...
                case DbusResult::NoService:
                    return ipmi::responseDestinationUnavailable();
                case DbusResult::Error:
                    lg2::error("PEF: refusing partial update; baseline read "
    ...
    [[maybe_unused]] uint8_t blockSelector)
{
    ...
    switch (static_cast<PefParam>(paramSel))
    {
        ...
        case PefParam::PefStartupDelay:
            if (auto r = mapErr(dbusGetByteProp(ctx, pef::propStartupDelay, v)))
            {
                return *r;
            }
            ret.pack(v);
        ...
        case PefParam::PefAlertStartupDelay:
            if (auto r =
                    mapErr(dbusGetByteProp(ctx, pef::propAlertStartupDelay, v)))
            {
                return *r;
            }
            ret.pack(v);
...
void registerPefFunctions()
{
    ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::netFnSensor,
                          ...
                          ipmi::Privilege::User, ipmiGetPefCapabilities);
    ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::netFnSensor,
                          ipmi::sensor_event::cmdSetPefConfigurationParams,
                          ipmi::Privilege::Admin, ipmiSetPefConfigParams);
    ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::netFnSensor,
                          ipmi::sensor_event::cmdGetPefConfigurationParams,
                          ipmi::Privilege::Operator, ipmiGetPefConfigParams);
}
