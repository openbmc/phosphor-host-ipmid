...
inline void setOffset(uint8_t offset, ipmi::sensor::GetSensorResponse* resp)
{
    if (offset > 7)
    {
        resp->thresholdLevelsStates       |= 1 << (offset - 8);
    }
    else
    {
        resp->discreteReadingSensorStates |= 1 << offset;
    }
}
...
{
    resp->readingState          = false;
    resp->scanningState         = true;
    resp->allEventMessagesState = false;
}
