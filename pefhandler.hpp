 ...
 *
 * Byte layout reference (IPMI 2.0 sec 17.7 Table 17-2) for the 20-byte entry:
 *   [0]  Filter Configuration   (bit 7 = enable)
 *   [1]  Event Filter Action    (bit 4 = OEM action)
 *   [2]  Alert Policy Number    (bits [6:4] = group control selector,
 *                               bits [3:0] = alert policy number)
 *   [3]  Event Severity
 *   [4]  Generator ID byte 1    (slave/SW addr; 0xFF = match any)
 *   [5]  Generator ID byte 2    (channel# | LUN; 0xFF = match any)
 *   [6]  Sensor Type            (0xFF = match any)
 *   [7]  Sensor Number          (0xFF = match any)
 *   [8]  Event/Reading Type     (event trigger; 0xFF = match any)
...
inline constexpr const char* pefMgrPath = "/xyz/openbmc_project/inventory/pef";
...
inline constexpr const char* dbusPropsIntf = "org.freedesktop.DBus.Properties";
