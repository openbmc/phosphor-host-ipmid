                                       ...
                                       bool& bSigned)
{
    ...
    if (max <= min)
    {
        log<level::DEBUG>("getSensorAttributes: Max must be greater than min");
        return false;
    }
    ...
    while (mDouble > maxInt10)
    {
        if (rExp >= maxInt4)
        {
            log<level::DEBUG>("rExp Too big, Max and Min range too far",
                              entry("REXP=%d", rExp));
            return false;
        }
        mDouble /= 10;
    ...
    while (((mDouble - floor(mDouble)) / mDouble) > (1.0 / 255))
    {
        if (rExp <= minInt4)
        {
            log<level::DEBUG>("rExp Too Small, Max and Min range too close");
            return false;
        }
