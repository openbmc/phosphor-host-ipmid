...
class IpmiHandler final : public HandlerBase
{
  ...
  private:
        ...
        executeCallback(message::Request::ptr request) override
    {
        ...
        {
            lg2::info(
                "Handler produced exception, NetFn: {NETFN}, Cmd: {CMD}: {ERROR}",
        ...
        {
            lg2::error(
                "Handler failed to catch exception, NetFn: {NETFN}, Cmd: {CMD}: {ERROR}",
                "NETFN", lg2::hex, request->ctx->netFn, "CMD", lg2::hex,
                request->ctx->cmd, "ERROR", e);
...
class IpmiHandler<ipmid_callback_t> final : public HandlerBase
{
  ...
  private:
        ...
        executeCallback(message::Request::ptr request) override
    {
        ...
        {
            lg2::info(
                "Legacy Handler produced exception, NetFn: {NETFN}, Cmd: {CMD}: {ERROR}",
                "NETFN", lg2::hex, request->ctx->netFn, "CMD", lg2::hex,
                request->ctx->cmd, "ERROR", e);
            return errorResponse(request, e.code());
        }
        ...
        {
            lg2::error(
                "Legacy Handler failed to catch exception, NetFn: {NETFN}, Cmd: {CMD}: {ERROR}",
                "NETFN", lg2::hex, request->ctx->netFn, "CMD", lg2::hex,
                request->ctx->cmd, "ERROR", e);
            return errorResponse(request, ccUnspecifiedError);
        }
...
class IpmiHandler<oem::Handler> final : public HandlerBase
{
  ...
  private:
        ...
        executeCallback(message::Request::ptr request) override
    {
        ...
        {
            lg2::info(
                "Legacy OEM Handler produced exception, NetFn: {NETFN}, Cmd: {CMD}: {ERROR}",
                "NETFN", lg2::hex, request->ctx->netFn, "CMD", lg2::hex,
                request->ctx->cmd, "ERROR", e);
            return errorResponse(request, e.code());
        }
        ...
        {
            lg2::error(
                "Legacy OEM Handler failed to catch exception, NetFn: {NETFN}, Cmd: {CMD}: {ERROR}",
                "NETFN", lg2::hex, request->ctx->netFn, "CMD", lg2::hex,
                request->ctx->cmd, "ERROR", e);
            return errorResponse(request, ccUnspecifiedError);
        }
        ...
        catch (...)
        {
            const char* what = currentExceptionType();
            lg2::error(
                "Legacy failed to catch exception, NetFn: {NETFN}, Cmd: {CMD}: {ERROR}",
                "NETFN", lg2::hex, request->ctx->netFn, "CMD", lg2::hex,
                request->ctx->cmd, "ERROR", what);
            return errorResponse(request, ccUnspecifiedError);
        }
