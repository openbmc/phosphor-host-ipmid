  ...
  private:
    ...
    void ipmiAuEvent(int32_t rc, uint8_t netFn, uint8_t cmd)
    {
        if (ipmiAuditEnabled == std::nullopt)
        {
            ...
            reply.read(prop);
            ipmiAuditEnabled = std::get<bool>(prop);
        }
        ...
        if (*ipmiAuditEnabled)
        {
            if (ipmiAllowList == std::nullopt)
            {
                ...
                auto reply = bus.call(method);
                std::variant<
                    std::vector<std::tuple<uint8_t, uint8_t, uint32_t>>>
                    prop;
                reply.read(prop);
                ipmiAllowList = std::move(
                    std::get<
                        std::vector<std::tuple<uint8_t, uint8_t, uint32_t>>>(
                        prop));
            }

            for (auto& ialEntry : *ipmiAllowList)
            {
                    ...
                    (netFn == std::get<1>(ialEntry)))
                {
                    ...
                    // auto username = std::string(pw->pw_name);
