                       ...
                       uint8_t* outBytes, size_t* outBytesLen)
{
    ...
    if (!retval)
    {
        log<level::DEBUG>("EVP_CipherInit_ex failed", entry("RET_VAL=%d", retval));
        return -1;
    }
    ...
    {
        outLen += outEVPLen;
        if ((retval = EVP_CipherFinal(ctx.get(), outBytes + outLen, &outEVPLen)))
        {
            outLen += outEVPLen;
            *outBytesLen = outLen;
        }
        ...
        {
            log<level::DEBUG>("EVP_CipherFinal fails", entry("RET_VAL=%d", retval));
            return -1;
        }
    }
    else
    {
        log<level::DEBUG>("EVP_CipherUpdate fails", entry("RET_VAL=%d", retval));
        return -1;
    }
    return 0;
}
...
void PasswdMgr::initPasswordMap(void)
{
    ...
    std::ifstream keyFile(ENCRYPT_KEY_FILE_NAME, std::ios::in | std::ios::binary);
    if (!keyFile.is_open())
    {
    ...
    char* outPtr = (char*)outBytes.data();
