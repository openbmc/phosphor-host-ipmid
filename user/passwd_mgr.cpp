...
static const char* passwdFileName = "/etc/ipmi-pass";
...
int PasswdMgr::encryptDecryptData(uint8_t isEncrypt, const EVP_CIPHER* cipher,
...
int PasswdMgr::readPasswdFileData(std::vector<uint8_t>& outBytes)
{
    ...
    if (encryptDecryptData(0, EVP_aes_128_cbc(), key.data(), keyLen, iv, ivLen,
                           inBytes, inBytesLen, mac, &macLen, outBytes.data(),
                           &outBytesLen) != 0)
    {
...
int PasswdMgr::updatePasswdSpecialFile(const std::string& userName)
{
    ...
    if (inBytesLen != 0)
    {
        if (encryptDecryptData(1, EVP_aes_128_cbc(), key.data(), keyLen,
