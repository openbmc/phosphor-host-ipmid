                                     ...
                                     const DbusInterface& interface)
{
    ...
    for (const auto& interface : interfaceList)
    {
        for (const auto& property : interface.second)
        {
            ...
            for (const auto& value : property.second.offsetValueMap)
            {
                     ...
                     uint8_t data)
{
    ...
    for (const auto& property : interface->second)
    {
        ...
        const auto& iter = property.second.offsetValueMap.find(data);
        if (iter == property.second.offsetValueMap.end())
        {
                     ...
                     const Info& sensorInfo)
{
    ...
    {
        SkipAssertion skip = SkipAssertion::NONE;
        ...
        for (const auto& property : interface.second)
        {
            ...
            for (const auto& value : property.second.offsetValueMap)
            {
                ...
                if (value.second.skip != SkipAssertion::NONE)
                {
                    skip = value.second.skip;
                }
            }

            for (const auto& value : property.second.preReqOffsetValueMap)
            {
                if (assertionSet.test(value.first))
                {
                    result = result && value.second.assert.get<bool>();
            ...
            if (valid)
            {
                ...
                if ((skip != SkipAssertion::NONE) &&(
                    ((skip == SkipAssertion::FALSE)&&(result == false))||
                    ((skip == SkipAssertion::TRUE)&&(result == true))))
                {
                    return IPMI_CC_OK;
                }
