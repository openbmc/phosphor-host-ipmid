                                     ...
                                     const DbusInterface& interface)
{
    ...
    for (const auto& interface : interfaceList)
    {
        for (const auto& property : interface.second)
        {
            ...
            for (const auto& value : std::get<1>(property.second))
            {
                     ...
                     uint8_t data)
{
    ...
    for (const auto& property : interface->second)
    {
        ...
        const auto& iter = std::get<1>(property.second).find(data);
        if (iter == std::get<1>(property.second).end())
        {
                     ...
                     const Info& sensorInfo)
{
    ...
    for (const auto& interface : sensorInfo.propertyInterfaces)
    {
        ...
        for (const auto& property : interface.second)
        {
            ...
            for (const auto& value : std::get<0>(property.second))
            {
                if (assertionSet.test(value.first))
                {
                    result = result && value.second.assert.get<bool>();
