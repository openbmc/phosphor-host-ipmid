...
ipmi_ret_t discreteSignal(IPMIUpdateData& msg, const auto& interfaceList, uint8_t data)
{
    auto& interface = interfaceList.begin();
    msg.append(interface.first);
    for (const auto& property : interface.second)
    {
        msg.append(property.second);
...
ipmi_ret_t sendData(IPMIUpdateData& msg, const auto& interfaceList, Value data)
{
    auto& interface = interfaceList.begin();
    msg.append(interface.first);
    for (const auto& property : interface.second)
...
IPMIUpdateData makeDbusMsg(std::string& updateInterface,
...
ipmi_ret_t discreteSignal(IPMIUpdateData& msg, auto& interface, uint8_t data)
{
    return IPMI_CC_OK;;
}

ipmi_ret_t sendData(IPMIUpdateData& msg, auto& interface, Value data)
{
    return IPMI_CC_OK;
}
