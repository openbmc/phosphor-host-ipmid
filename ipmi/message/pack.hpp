...
void PackByte(uint8_t* pointer, const NumericType& i)
{
    if constexpr (byteIndex > 0)
    {
        *pointer = static_cast<uint8_t>(i >> (8 * (byteIndex - 1)));
        PackByte<byteIndex - 1, NumericType>(pointer + 1, i);
    }
}
...
struct PackSingle
{
    ...
    static int op(Response& m, T& t)
    {
        ...
        m.raw.resize(m.raw.size() + sizeof(T));
        uint8_t* out = m.raw.data() + m.raw.size() - sizeof(T);
        PackByte<sizeof(T), T>(out, t);
        return 0;
    }
};
...
{
    static int op(Response& m, std::vector<uint8_t>& t)
    {
        m.raw.reserve(m.raw.size() + t.size());
        m.raw.insert(m.raw.end(), t.begin(), t.end());
        return 0;
    }
};
