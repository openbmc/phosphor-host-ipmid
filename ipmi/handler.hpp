...
class IpmiHandler : public HandlerBase
{
  ...
  private:
        ...
        executeCallback(message::Request::ptr request) override
    {
        ...
        std::forward_list<InputArgsType> inputArgs;
        if constexpr (std::tuple_size<InputArgsType>::value > 0)
        {
            ...
            {
                inputArgs.push_front(std::tuple_cat(
            ...
            {
                inputArgs.push_front(std::move(unpackArgs));
            }
        }
        else
        {
        ...
        // ipmi::RspType
        auto result = std::apply(handler_, inputArgs.front());
