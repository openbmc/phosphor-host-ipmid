...
/** @brief After sending SMS_ATN to the Host, Host comes down and asks
 *         why an 'SMS_ATN` was sent. IPMI protocol demands that BMC send
 *         'There is a Message to be Read` as reponse. Host then comes down
 *         asks for Message and the specified commands and data would go
 *         as data conforming to IPMI spec.
 */
 ...
 */
using IpmiCmdData = std::pair<uint8_t, uint8_t>;

/** @brief implementation specific callback function to be invoked
...
/** @brief Tuple encapsulating above 2 to enable using Manager by different
