...
{
    ipmi_ret_t rc = IPMI_CC_OK;
    FruInvenAreaInfoRequest_t* reqptr =
        reinterpret_cast<FruInvenAreaInfoRequest_t*>(request);
    try
    {
        sdbusplus::bus::bus bus{ipmid_get_sd_bus_connection()};
        FruAreaData fruArea = getFruAreaData(bus, reqptr->fruID);
        uint16_t size = static_cast<uint16_t>(fruArea.size());
        FruInvenAreaInfoResponse_t resp;
        resp.sizems = size >> 8;
        resp.sizels = size;
...
{
    ipmi_ret_t rc = IPMI_CC_OK;
    ReadFruDataRequest_t* reqptr = (ReadFruDataRequest_t*)request;
    uint16_t offset = ((uint16_t)reqptr->offsetMS) << 8 | reqptr->offsetLS;
    try
    {
        sdbusplus::bus::bus bus{ipmid_get_sd_bus_connection()};
        FruAreaData fruArea = getFruAreaData(bus, reqptr->fruID);
        size_t size = fruArea.size();
