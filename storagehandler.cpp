 ...
 *  - reqCount  - Count to read
 *
 * @returns IPMI completion code plus response data
 *  - respCount  - Response data Count.
 *  - fruData    - response data
 */
ipmi::RspType<uint8_t,              // respCount
              std::vector<uint8_t>> // fruData
    ipmiStorageReadFruData(uint8_t fruID, uint16_t offset, uint8_t reqCount)
{
    auto iter = frus.find(fruID);
    if (iter == frus.end())
    {
    ...
    try
    {
        ...
        if ((offset + reqCount) <= size)
        {
            respCount = reqCount;
        }
        else
        {
            respCount = size - offset;
        }

        std::vector<uint8_t> fruData;
        fruData.assign(fruArea.begin() + offset,
                       fruArea.begin() + offset + respCount);

        return ipmi::responseSuccess(respCount, fruData);
    }
    catch (const InternalFailure& e)
    {
        log<level::ERR>(e.what());
