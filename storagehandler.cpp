...
// Cache of event log object paths
                      ...
                      ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    ipmi::sel::readLoggingObjectPaths(objectPaths);
    ...
    {
        responseData->entries = static_cast<uint16_t>(objectPaths.size());
                       ...
                       ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    else
    {
        ...
        iter = std::find(objectPaths.begin(), objectPaths.end(), objPath);
        if (iter == objectPaths.end())
        {
            *data_len = 0;
            return IPMI_CC_SENSOR_INVALID;
        }
    ...
    if(iter != objectPaths.end())
    {
        ...
        else
        {
            ...
            record.nextRecordID = static_cast<uint16_t>
                     (std::stoi(std::string(path.filename().c_str())));
        }
    }
    else
    {
