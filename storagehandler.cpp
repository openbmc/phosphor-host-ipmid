        ...
        ipmi_context_t context)
{
    ...
    try
    {
        ...
        {
            std::copy((fruArea.begin() + offset), fruArea.end(), 
                static_cast<uint8_t*>(response));
            *data_len = count;
        }
        else
        {
            std::cerr << "Invalid offset " << offset << " and count " << 
                count << ". Out of range " << std::endl;
            log<level::ERR>("Invalid offset and count. Out of range",
                entry("Offset=%d Count=%d", offset, count));
