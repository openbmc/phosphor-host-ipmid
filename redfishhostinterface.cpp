                                           ...
                                           const bool& pValue)
{
    ...
    try
    {
        ipmi::setDbusProperty(*ctx->bus, ipmi::userMgrService,
                              ipmi::userObjBasePath, credBootstrappingInf,
                              credBootstrapEnabledProp, pValue);
    }
    catch (const std::exception& e)
    {
        lg2::error(
...
/** @brief Mapping the CredentialBootstrapping's roleId to the bootStrapAccount
 *         user Privilege
 *
 *  @returns bootStrapAccount user previlege, default PRIVILEGE_ADMIN
 */
inline uint8_t getPrivilegeFromRoleId(std::string_view role)
{
    if (role ==
 ...
 *  @returns bootStrapAccount user previlege, default PRIVILEGE_ADMIN
 */
inline uint8_t getRedfishHostInterfacePriv(ipmi::Context::ptr ctx)
{
    std::string service;
    uint8_t priv = PRIVILEGE_ADMIN;

    try
    {
        ipmi::Value bootstrapProperty = ipmi::getDbusProperty(
            *ctx->bus, ipmi::userMgrService, ipmi::userObjBasePath,
            credBootstrappingInf, credBootstrapRoleIdProp);
        priv = getPrivilegeFromRoleId(std::get<std::string>(bootstrapProperty));
    }
    catch (const std::exception& e)
    {
        lg2::error(
            "Can't get value of {PROP} property in the {INF} interface at path {PATH} error {ERROR}",
            "PROP", credBootstrapRoleIdProp, "INF", credBootstrappingInf,
            "PATH", ipmi::userObjBasePath, "ERROR", e);
        return PRIVILEGE_ADMIN;
    }
