...
{
    // We need the new VLAN interface first since our configurations
    // get applied to this object
    ...
    auto newParams = getChannelParams(bus, params.id);
    if (!newParams)
    {
    ...
    {
        auto prefix = conf.prefix.value_or(DEFAULT_PREFIX);
        auto protocol =
