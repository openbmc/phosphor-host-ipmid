    ...
    *data_len = 0;
    ...
    switch (param)
    {
        ...
        case LanParam::IPV6_DYNAMIC_ADDRESSES:
        {
            ...
            try
            {
                ...
                for (const auto& obj : objs)
                {
                    ...
                    if (param == LanParam::IPV6_STATIC_ADDRESSES)
                    {
                        ...
                        {
                            data.source_type |=
                                static_cast<uint8_t>(SourceType::Static);
                        }
                        else
                        {
                            continue;
                    ...
                    if (param == LanParam::IPV6_DYNAMIC_ADDRESSES)
                    {
                        ...
                        {
                            data.source_type |=
                                static_cast<uint8_t>(SourceType::DHCP);
                        }
                        else if (origin == IP::AddressOrigin::SLAAC)
                        {
                            data.source_type |=
                                static_cast<uint8_t>(SourceType::SLAAC);
                        }
                        else
                        {
                            continue;
                    ...
                    data.source_type |= 0xb1000000; // Enabled
            ...
            {
                std::fprintf(stderr, "Got exception: %s\n", e.what());
                rc = IPMI_CC_UNSPECIFIED_ERROR;
            }
            catch (...)
            {
                std::fprintf(stderr, "Got unknown error type\n");
                rc = IPMI_CC_UNSPECIFIED_ERROR;
            }
            break;
        }
