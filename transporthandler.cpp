            ...
            case LAN_PARM_IPSRC:
            {
                ...
                if (lan_set_in_progress == SET_COMPLETE)
                {
                    try
                    {
                        ...
                        // get the IP object and determine the network interface.
                        auto ipObject = ipmi::getDbusObject(bus,
                                ipmi::network::IP_INTERFACE,
                                ipmi::network::ROOT,
                                ipmi::network::IP_TYPE);

                        fs::path ipObjectPath = ipObject.first;

                        // ip object path would be like this.
                        // /xyz/openbmc_project/network/<interface>/<ipv4|6>/<id>
                        networkInterfacePath =
                            std::move(ipObjectPath.parent_path().parent_path());
                    ...
                    catch (InternalFailure& e)
                    {
                        ...
                        networkInterfacePath =
                            std::move(networkInterfaceObject.first);
                    }
