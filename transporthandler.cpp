                                  ...
                                  ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    constexpr auto networkTimeout = 10s;
    ...
    switch (reqptr->parameter)
    {
        ...
        case LAN_PARM_INPROGRESS:
        {
            if (reqptr->data[0] == SET_COMPLETE)
            {
                ...
                auto restartTime = duration_cast<microseconds>(networkTimeout);
                // start/restart the timer
                networkTimer->startTimer(restartTime);
            }
...
void commitNetworkChanges()
{
    ...
    //TODO :- channel number should have come with the caller
    //        but currently timer callback doesn't take the
    //        parameters,we need to enhance the functionality
    //        of timer.
    //        https://github.com/openbmc/openbmc/issues/2993

    auto channel = channelOne;
    auto ethdevice = ipmi::network::ChanneltoEthernet(channel);
    if (ethdevice.empty())
    {
