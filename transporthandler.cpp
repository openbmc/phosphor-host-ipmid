 ...
 *  @return The netmask
 */
struct in_addr prefixToNetmask(uint8_t prefix)
{
    if (prefix > 32)
    {
 ...
 *
 *  @param[in] netmask - The prefix length
 *  @return The prefix length
 */
uint8_t netmaskToPrefix(struct in_addr netmask)
{
...
ipmi_ret_t getNetworkData(uint8_t lan_param, uint8_t* data, int channel)
{
    ...
    try
    {
        switch (static_cast<LanParam>(lan_param))
        {
            ...
            {
                uint8_t prefix = 0;
                if (channelConf->prefix == INVALID_PREFIX)
                {
                    try
                    {
                        auto ipObjectInfo =
                ...
                struct in_addr netmask = prefixToNetmask(prefix);
        ...
        if (!channelConf->dhcpEnabled.value_or(false))
        {
                 ...
                 // or partially given data.
            {
                ...
                try
                {
                    ...
                    prefix = channelConf->prefix == INVALID_PREFIX
