...
enum class IPFamilySupportFlag : uint8_t
{
    IPv6Only = 0,
    DualStack = 1,
    IPv6Alerts = 2,
...
enum class IPv6RouterControlFlag : uint8_t
{
    Static = 0,
    Dynamic = 1,
};
...
auto channelCall(uint8_t channel, Args&&... args)
{
    ...
    auto params = getChannelParamsOrError(bus, channel);
...
IPv6Source originToSourceType(IP::AddressOrigin origin)
{
    switch (origin)
    {
        ...
        default:
        {
            ...
            elog<InternalFailure>();
            // TODO delete return for phosphor-logging change
            return IPv6Source::Static;
        }
    }
}
                     ...
                     uint16_t vlan)
{
    ...
    auto neighbor4 = getGatewayNeighbor<AF_INET>(bus, params);
    auto neighbor6 = getGatewayNeighbor<AF_INET6>(bus, params);
                 ...
                 message::Payload& req)
{
    ...
    switch (static_cast<LanParam>(parameter))
    {
        ...
        case LanParam::IPv6RouterControl:
        {
            ...
            {
                expected[static_cast<uint8_t>(IPv6RouterControlFlag::Dynamic)] =
                    1;
            }
            else
            {
                expected[static_cast<uint8_t>(IPv6RouterControlFlag::Static)] =
                    1;
            }
            if (expected != control)
            {
                                 ...
                                 uint8_t parameter, uint8_t set, uint8_t block)
{
    ...
    switch (static_cast<LanParam>(parameter))
    {
        ...
        case LanParam::IPFamilySupport:
        {
            ...
            support[static_cast<uint8_t>(IPFamilySupportFlag::IPv6Only)] = 0;
            support[static_cast<uint8_t>(IPFamilySupportFlag::DualStack)] = 1;
            support[static_cast<uint8_t>(IPFamilySupportFlag::IPv6Alerts)] = 1;
            ret.pack(support);
            return responseSuccess(std::move(ret));
        }
        case LanParam::IPFamilyEnables:
        ...
        case LanParam::IPv6Status:
        {
            ...
            std::bitset<8> support;
            support[static_cast<uint8_t>(IPv6StatusFlag::DHCP)] = 1;
            support[static_cast<uint8_t>(IPv6StatusFlag::SLAAC)] = 1;
            ret.pack(support);
            return responseSuccess(std::move(ret));
        }
        case LanParam::IPv6StaticAddresses:
        ...
        case LanParam::IPv6RouterControl:
        {
            ...
            if (channelCall<getDHCPProperty>(channel))
            {
                control[static_cast<uint8_t>(IPv6RouterControlFlag::Dynamic)] =
                    1;
            }
            else
            {
                control[static_cast<uint8_t>(IPv6RouterControlFlag::Static)] =
                    1;
            }
            ret.pack(control);
            return responseSuccess(std::move(ret));
