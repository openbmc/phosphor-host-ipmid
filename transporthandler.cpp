 ...
 */
template <typename T>
static T getEthProp(ipmi::Context::ptr& ctx, const ChannelParams& params,
                    const std::string& prop)
{
    T value;
    ipmi::getDbusProperty(ctx, params.service, params.logicalPath,
                          INTF_ETHERNET, prop, value);
    return value;
}
template <typename T>
static void setEthProp(ipmi::Context::ptr& ctx, const ChannelParams& params,
                       const std::string& prop, const T& t)
{
    ipmi::setDbusProperty(ctx, params.service, params.logicalPath,
 ...
 */
void setMACProperty(ipmi::Context::ptr& ctx, const ChannelParams& params,
                    stdplus::EtherAddr mac)
{
    ipmi::setDbusProperty(ctx, params.service, params.ifPath, INTF_MAC,
                          ...
                          const std::string& path)
{
    ...
    ipmi::callDbusMethod(ctx, service, path, ipmi::DELETE_INTERFACE, "Delete");
}
 ...
 */
template <int family>
void createIfAddr(ipmi::Context::ptr& ctx, const ChannelParams& params,
                  typename AddrFamily<family>::addr address, uint8_t prefix)
{
    ...
    ipmi::callDbusMethod(ctx, params.service, params.logicalPath,
                         INTF_IP_CREATE, "IP", protocol, tsh(address), prefix,
                         "");
}
 ...
 */
void reconfigureIfAddr4(ipmi::Context::ptr& ctx, const ChannelParams& params,
                        ...
                        std::optional<uint8_t> prefix)
{
    ...
    if (ifaddr)
    {
        ...
        deleteObjectIfExists(ctx, params.service, ifaddr->path);
    }
    addr = address.value_or(addr);
    if (addr != stdplus::In4Addr{})
    {
        createIfAddr<AF_INET>(ctx, params, addr,
...
std::optional<IfNeigh<family>>
    findGatewayNeighbor(ipmi::Context::ptr& ctx, const ChannelParams& params,
...
template <int family>
std::optional<IfNeigh<family>>
    getGatewayNeighbor(ipmi::Context::ptr& ctx, const ChannelParams& params)
{
...
template <int family>
void reconfigureGatewayMAC(ipmi::Context::ptr& ctx, const ChannelParams& params,
                           stdplus::EtherAddr mac)
{
    auto gateway = getGatewayProperty<family>(ctx, params);
    if (!gateway)
    ...
    createNeighbor<family>(ctx, params, *gateway, mac);
}
 ...
 */
void deconfigureIfAddr6(ipmi::Context::ptr ctx, const ChannelParams& params,
                        uint8_t idx)
{
...
{
    deconfigureIfAddr6(ctx, params, idx);
    createIfAddr<AF_INET6>(ctx, params, address, prefix);
}
 ...
 *  @param[in] params - The parameters for the channel
 */
void deconfigureChannel(ipmi::Context::ptr& ctx, ChannelParams& params)
{
    ...
    for (const auto& [path, impls] : objs)
    {
        ...
        for (const auto& [service, intfs] : impls)
        {
            deleteObjectIfExists(ctx, service, path);
        }
    ...
    setEthProp(ctx, params, "DHCP4", false);
    setEthProp(ctx, params, "DHCP6", false);
    setEthProp(ctx, params, "IPv6AcceptRA", false);
}
 ...
 */
void createVLAN(ipmi::Context::ptr& ctx, ChannelParams& params, uint16_t vlan)
{
    if (vlan == 0)
    {
        return;
 ...
 */
void reconfigureVLAN(ipmi::Context::ptr& ctx, ChannelParams& params,
                     uint16_t vlan)
{
    ...
    bool dhcp4 = getEthProp<bool>(ctx, params, "DHCP4");
    bool dhcp6 = getEthProp<bool>(ctx, params, "DHCP6");
    bool ra = getEthProp<bool>(ctx, params, "IPv6AcceptRA");
    ...
    deconfigureChannel(ctx, params);
    createVLAN(ctx, params, vlan);

    // Re-establish the saved settings
    setEthProp(ctx, params, "DHCP4", dhcp4);
    setEthProp(ctx, params, "DHCP6", dhcp6);
    setEthProp(ctx, params, "IPv6AcceptRA", ra);
    if (ifaddr4)
    {
        createIfAddr<AF_INET>(ctx, params, ifaddr4->address, ifaddr4->prefix);
    }
    for (const auto& ifaddr6 : ifaddrs6)
    {
        createIfAddr<AF_INET6>(ctx, params, ifaddr6.address, ifaddr6.prefix);
    }
    if (neighbor4)
    {
    ...
    if (neighbor6)
    {
        createNeighbor<AF_INET6>(ctx, params, neighbor6->ip, neighbor6->mac);
    }
}
                    ...
                    uint8_t parameter, message::Payload& req)
{
    ...
    switch (static_cast<LanParam>(parameter))
    {
        ...
        case LanParam::IP:
        {
            if (channelCall<getEthProp<bool>>(ctx, channel, "DHCP4"))
            {
        ...
        case LanParam::IPSrc:
        {
            ...
            switch (static_cast<IPSrc>(static_cast<uint8_t>(flag)))
            {
                case IPSrc::DHCP:
                    ...
                    // subcommand.
                    channelCall<setEthProp<bool>>(ctx, channel, "DHCP4", true);
                    return responseSuccess();
                case IPSrc::Unspecified:
                case IPSrc::Static:
                    channelCall<setEthProp<bool>>(ctx, channel, "DHCP4", false);
                    return responseSuccess();
                case IPSrc::BIOS:
                case IPSrc::BMC:
                    return responseInvalidFieldRequest();
        ...
        case LanParam::MAC:
        {
            ...
            unpackFinal(req);
            channelCall<setMACProperty>(ctx, channel, mac);
            return responseSuccess();
        }
        case LanParam::SubnetMask:
        {
            ...
            unpackFinal(req);
            channelCall<reconfigureIfAddr4>(ctx, channel, std::nullopt, pfx);
            return responseSuccess();
        }
        ...
        case LanParam::Gateway1MAC:
        {
            ...
            unpackFinal(req);
            channelCall<reconfigureGatewayMAC<AF_INET>>(ctx, channel,
                                                        gatewayMAC);
            return responseSuccess();
        }
        case LanParam::VLANId:
        {
            ...
            channelCall<reconfigureVLAN>(ctx, channel, vlan);
            return responseSuccess();
        }
        ...
        case LanParam::IPv6StaticAddresses:
        {
            ...
            if (enabled)
            {
                ...
                channelCall<reconfigureIfAddr6>(ctx, channel, set, ip, prefix);
            }
            else
            {
                channelCall<deconfigureIfAddr6>(ctx, channel, set);
            }
            return responseSuccess();
        }
        ...
        case LanParam::IPv6RouterControl:
        {
            ...
            channelCall<setEthProp<bool>>(ctx, channel, "IPv6AcceptRA",
                                          enableRA);
            channelCall<setEthProp<bool>>(ctx, channel, "DHCP6", enableRA);
            return responseSuccess();
        }
        case LanParam::IPv6StaticRouter1IP:
        {
            ...
            unpackFinal(req);
            channelCall<setGatewayProperty<AF_INET6>>(ctx, channel, gateway);
            return responseSuccess();
        }
                                 ...
                                 uint8_t parameter, uint8_t set, uint8_t block)
{
    ...
    switch (static_cast<LanParam>(parameter))
    {
        ...
        case LanParam::IPSrc:
        {
            auto src = channelCall<getEthProp<bool>>(ctx, channel, "DHCP4")
                           ? IPSrc::DHCP
                           : IPSrc::Static;
        ...
        case LanParam::IPv6RouterControl:
        {
            ...
            control[IPv6RouterControlFlag::Dynamic] =
                channelCall<getEthProp<bool>>(ctx, channel, "IPv6AcceptRA");
        ...
        case LanParam::IPv6StaticRouter1IP:
        {
            ...
            if (!channelCall<getEthProp<bool>>(ctx, channel, "IPv6AcceptRA"))
            {
                gateway =
