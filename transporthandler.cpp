 ...
 */
std::optional<stdplus::EtherAddr> getMACProperty(ipmi::Context::ptr& ctx,
                                                 const ChannelParams& params)
{
    std::string macAddr;
    auto ec = ipmi::getDbusProperty(ctx, params.service, params.ifPath,
                                    INTF_MAC, "MACAddress", macAddr);
    if (ec)
    {
        return std::nullopt;
    }
                                          ...
                                          ChannelParams& params, uint16_t vlan)
{
    ...
    ec = setEthProp(ctx, parentIntParams, "DHCP4", dhcp4);
    if (ec)
    {
        lg2::error("Failed to set DHCP4 property for {IFNAME}: {ERR}", "IFNAME",
                   parentIntParams.ifname, "ERR", ec.message());
        return ec;
    }
    ec = setEthProp(ctx, parentIntParams, "DHCP6", dhcp6);
    if (ec)
    {
        lg2::error("Failed to set DHCP6 property for {IFNAME}: {ERR}", "IFNAME",
                   parentIntParams.ifname, "ERR", ec.message());
    ...
    ec = setEthProp(ctx, parentIntParams, "IPv6AcceptRA", ra);
    if (ec)
    {
        lg2::error("Failed to set IPv6AcceptRA property for {IFNAME}: {ERR}",
                   "IFNAME", parentIntParams.ifname, "ERR", ec.message());
        return ec;
    }
                                 ...
                                 uint8_t parameter, uint8_t set, uint8_t block)
{
    ...
    switch (static_cast<LanParam>(parameter))
    {
        ...
        case LanParam::MAC:
        {
            ...
            if (!mac)
            {
                lg2::error("Failed to get MAC address: {ERROR}", "ERROR",
                           ec.message());
                return responseInvalidFieldRequest();
            }
