...
{
    constexpr auto oneTimeSupportedCount = 2;
    ...
    if (!count)
    {
        // If there is no objects implementing the requested interface,
        // that must be an error.
        log<level::ERR>("Interface objects not found",
                        entry("INTERFACE=%s", iface.c_str()));
        elog<InternalFailure>();
    }
    else if (count < oneTimeSupportedCount)
    {
    ...
    // one-time setting, consider the other the regular setting.
    size_t index = 0;
    ...
    // if so. Otherwise return the path for the regular setting.
