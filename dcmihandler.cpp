...
// When DHCP Option 12 is enabled the string "SendHostNameEnabled=true" will be
// added into n/w configuration
...
bool getDHCPOption(std::string prop)
{
    ...
    auto value = ipmi::getDbusProperty(bus, service, dhcpObj, dhcpIntf,
                    prop.c_str());
...
void setDHCPOption(std::string prop, bool value)
{
    ...
    auto service = ipmi::getService(bus, dhcpIntf, dhcpObj);
    ipmi::setDbusProperty(bus, service, dhcpObj, dhcpIntf, prop.c_str(), value);
}
    ...
    try
    {
        ...
        switch (requestData->paramSelect)
        {
            case static_cast<unsigned int>(
                                dcmi::DCMIConfigParameters::ActivateDHCP):
            ...
            case static_cast<unsigned int>(
                                dcmi::DCMIConfigParameters::DiscoveryConfig):
            ...
            case static_cast<unsigned int>(
                                dcmi::DCMIConfigParameters::DHCPTiming1):
                break;
            case static_cast<unsigned int>(
                                dcmi::DCMIConfigParameters::DHCPTiming2):
                break;
            case static_cast<unsigned int>(
                                dcmi::DCMIConfigParameters::DHCPTiming3):
                break;
            default:
                return IPMI_CC_INVALID;
        }
