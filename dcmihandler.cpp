...
ipmi::RspType<uint8_t // Total Asset Tag Length
              >
    setAssetTag(ipmi::Context::ptr& ctx, uint8_t offset,
                const std::string& assetTagData)
{
    uint8_t tagLength = 0;

    // Verify offset to read and number of bytes to read are not exceeding the
    // range.
    if ((offset > dcmi::assetTagMaxOffset) ||
        (assetTagData.length() > dcmi::maxBytes) ||
        ((offset + assetTagData.length()) > dcmi::assetTagMaxSize))
    {
        return ipmi::responseParmOutOfRange();
    }
    ...
    try
    {
        ...
        assetTag.replace(offset, (assetTag.size() - offset),
                         assetTagData.c_str(), assetTagData.length());
