...
{
    /*
     * Writing Asset Tag to FRU follows steps:
    ...
    try
    {
        ...
        for (const auto& [path, serviceMap] : objectTree)
        {
            for (const auto& [service, intfs] : serviceMap)
            {
                ...
                // Invoke "Set" method to update PRODUCT_ASSET_TAG property
                dbus->async_method_call(
                    [](const boost::system::error_code ec) {
                        // Reset cache data even writing request is fail
                        assetTagCache = std::nullopt;
                        if (ec)
                        {
                            lg2::error("Error set dbus property: {ERROR}",
                                       "ERROR", ec.message());
                            return;
                        }
                    },
                    service, path, ipmi::PROP_INTF, ipmi::METHOD_SET,
                    fruDeviceIntf, "PRODUCT_ASSET_TAG",
                    std::variant<std::string>(assetTagCache.value()));
