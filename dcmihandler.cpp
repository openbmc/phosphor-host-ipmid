...
{
    auto data = parseJSONConfig(kDCMICapabilitiesConfig);

    return (kDCMIPowerMgmtSupported ==
            data.value(kDCMIPowerMgmtCapability, 0));
}
                               ...
                               ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    std::ifstream dcmiCapFile(dcmi::kDCMICapabilitiesConfig);
    ...
    for (auto cap : caps->second.capList)
    {
        ...
        if ((cap.length + cap.position) > CHAR_BIT)
        {
            ...
            if (cap.length > CHAR_BIT)
            {
                val &= dcmi::kMaxSELEntriesMask;
            }
            val <<= cap.position;
                                        ...
                                        uint8_t instance)
{
    ...
    auto data = parseJSONConfig(kDCMISensorsConfig);
                         ...
                         ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    try
    {
        if (!parsed)
        {
            config = dcmi::parseJSONConfig(dcmi::kDCMISensorsConfig);
            parsed = true;
        }
