...
static std::optional<uint16_t> readPower(ipmi::Context::ptr& ctx)
{
    ...
    if (!sensorFile.is_open())
    {
        lg2::error(
            ...
            "POWER_SENSOR_FILE", std::string{POWER_READING_SENSOR});
        return std::nullopt;
    }
    ...
    {
        lg2::error(
            "Failed to fetch service for D-Bus object, object path: {OBJECT_PATH}, interface: {INTERFACE}",
            "OBJECT_PATH", objectPath, "INTERFACE", dcmi::sensorValueIntf);
        return std::nullopt;
    }
    ...
    {
        lg2::error(
            "Failed to read power value from D-Bus object, object path: {OBJECT_PATH}, interface: {INTERFACE}",
            "OBJECT_PATH", objectPath, "INTERFACE", dcmi::sensorValueIntf);
        return std::nullopt;
    }
