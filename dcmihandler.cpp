...
constexpr auto SUPPORTED_DCMI_CAPS = 1;
constexpr auto MANDATORY_PLAT_ATTRIBUTES = 2;
constexpr auto OPTIONAL_PLAT_ATTRIBUTES = 3;
constexpr auto MANAGEABILITY_ACCESS_ATTRIBUTES = 4;
                       ...
                       ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    {
        //If bit position is beyond a byte or length of data
        //if beyond a byte, operate on a 16bit value.
        if ((cap.position / 8) || (cap.length > 8))
        {
            reinterpret_cast<uint16_t *>(
                responseData->data)[(cap.bytePosition - 1)/sizeof(uint16_t)] |=
