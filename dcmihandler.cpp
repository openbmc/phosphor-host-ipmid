...
int64_t getPowerReading(sdbusplus::bus::bus& bus)
{
    ...
    {
        log<level::ERR>("Failed to open sensor config file",
                        entry("SENSOR_FILE_PATH=%s", POWER_READING_SENSOR));
        elog<InternalFailure>();
    }
    if(objectPath.empty())
    {
        log<level::ERR>("Error in reading power sensor object path",
                        entry("SENSOR_FILE_PATH=%s", POWER_READING_SENSOR));
        elog<InternalFailure>();
    }
