...
{
    sdbusplus::bus::bus bus{ ipmid_get_sd_bus_connection() };
    auto value = ipmi::getDbusProperty(bus, networkServiceName,
    ...
    ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    auto resp_str_len = std::min(static_cast<std::size_t>(requestData->bytes),
    ...
    ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    *data_len = 0;
    ...
    try
    {
        ...
        /* replace part of string and mark all bytes after the last as \0 */
        auto rest_it = std::copy(requestData->data,
            requestData->data + requestData->bytes,
            begin(newCtrlIdStr) + requestData->offset);
        std::fill(rest_it, end(newCtrlIdStr), '\0');
