    ...
    getDCMICapabilities(uint8_t parameterSelector)
{
    ...
    uint8_t capData[caps->second.size] = {0};
    ...
    std::vector<uint8_t> capD;
    capD.insert(capD.begin(), &capData[0], &capData[caps->second.size]);

    return ipmi::responseSuccess(major, minor, paramRevision, capD);
}
