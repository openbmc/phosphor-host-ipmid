...
int64_t getPowerReading(sdbusplus::bus::bus& bus)
{
    ...
    //Read the sensor property value
    auto value = ipmi::getDbusProperty(
            bus, service, objectPath, SENSOR_VALUE_INTF, SENSOR_VALUE_PROP);
    int64_t power  = value.get<int64_t>();

    //Read the sensor scale value
    auto scale = ipmi::getDbusProperty(
            bus, service, objectPath, SENSOR_VALUE_INTF, SENSOR_SCALE_PROP);

    // Power reading needs to be scaled with the Scale value using the formula
    // Value * 10^Scale.
    power *= std::pow(10, scale.get<int64_t>());
