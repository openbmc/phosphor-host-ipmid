    ...
    ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    try
    {
        ...
        printf("%s(): offset %u, len %u: \"", __FUNCTION__, requestData->offset,
            requestData->bytes);
        for (size_t i = 0; i < requestData->bytes; i++)
            if (requestData->data[i] != '\0')
                printf("%c", requestData->data[i]);
            else
                printf("\\0");
        printf("\"\n");
