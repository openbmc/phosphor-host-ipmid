...
ipmi::RspType<uint8_t> ipmiAppGetMessageFlags()
{
    ...
    auto wdtObj = ipmi::getDbusObject(bus, wdt_intf);
    uint8_t preTimeoutInterruptsValue = std::get<uint8_t>(ipmi::getDbusProperty(
        bus, wdtObj.second, wdtObj.first, wdt_intf, "PreTimeoutInterrupt"));
