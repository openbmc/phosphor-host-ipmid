...
static ipmid::OemRouter* singletonUnderTest;
...
void MakeRouter()
{
    ...
    {
        singletonUnderTest = mutableOemRouter();
    }
    ASSERT_EQ(singletonUnderTest, mutableOemRouter());
}
...
void RegisterWithRouter(OemNumber oen, ipmi_cmd_t cmd, OemHandler cb)
{
    ActivateRouter();
    singletonUnderTest->registerHandler(oen, cmd, cb);
}

Byte msgPlain[] = { 0x56, 0x34, 0x12 };
Byte replyPlain[] = { 0x56, 0x34, 0x12, 0x31, 0x41 };
Byte msgPlus2[] = { 0x67, 0x45, 0x23, 0x10, 0x20 };
Byte msgBadOen[] = { 0x57, 0x34, 0x12 };

void RegisterTwoWays(ipmi_cmd_t *nextCmd)
{
    OemHandler f = [](ipmi_cmd_t cmd, const Byte* reqBuf,
                      Byte* replyBuf, size_t* dataLen)
    {
        ...
        // Generate reply.
        *dataLen = 2;
        memcpy(replyBuf, replyPlain + 3, *dataLen);
        return 0;
    };
    RegisterWithRouter(0x123456, 0x78, f);

    *nextCmd = IPMI_CMD_WILDCARD;
    OemHandler g = [nextCmd](ipmi_cmd_t cmd, const Byte* reqBuf,
                             Byte* replyBuf, size_t* dataLen)
    {
        // Check inputs
...
TEST(OemRouterTest, Singleton) {
    MakeRouter();
}

TEST(OemRouterTest, Activate) {
    lastNetFunction = 0;
    ActivateRouter();
}

TEST(OemRouterTest, SpecificCommandMatch) {
    ipmi_cmd_t cmd;
    Byte reply[256];
    size_t dataLen;
...
TEST(OemRouterTest, WildCardMatch) {
    ipmi_cmd_t cmd;
    Byte reply[256];
    size_t dataLen;
...
TEST(OemRouterTest, Rejectables) {
    ipmi_cmd_t cmd;
    Byte reply[256];
    size_t dataLen;
    ...
    // Message too short to include whiole OEN?
    dataLen = 2;
