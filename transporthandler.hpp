...
template <int family>
boost::system::error_code createNeighbor(
    ipmi::Context::ptr& ctx, const ChannelParams& params,
    typename AddrFamily<family>::addr address, stdplus::EtherAddr mac)
{
 ...
 */
template <int family>
boost::system::error_code setGatewayProperty(
    ipmi::Context::ptr& ctx, const ChannelParams& params,
    typename AddrFamily<family>::addr address)
{
    ...
    if (ec || neighbor)
    {
        ...
        if (!ec)
        {
            ec = createNeighbor<family>(ctx, params, address, neighbor->mac);
        }
    }
