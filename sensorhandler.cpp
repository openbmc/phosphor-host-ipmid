...
ipmi_ret_t ipmi_sen_get_sdr_info(ipmi_netfn_t netfn, ipmi_cmd_t cmd,
                                 ...
                                 ipmi_data_len_t data_len, ipmi_context_t context)
{
    get_sdr_info::GetSdrInfoResp* resp = (get_sdr_info::GetSdrInfoResp*)response;
    if (request == nullptr || get_sdr_info::request::get_count(request) == false)
    {
        // Get Sensor Count
    ...
    get_sdr_info::response::set_lun_present(0, &(resp->luns_and_dynamic_population));
    get_sdr_info::response::set_lun_not_present(1, &(resp->luns_and_dynamic_population));
    get_sdr_info::response::set_lun_not_present(2, &(resp->luns_and_dynamic_population));
    get_sdr_info::response::set_lun_not_present(3, &(resp->luns_and_dynamic_population));
    get_sdr_info::response::set_static_population(&(resp->luns_and_dynamic_population));
                                     ...
                                     ipmi_data_len_t data_len)
{
    ...
    if (info->sensorInterfaces.begin()->first == "xyz.openbmc_project.Sensor.Value")
    {
        ...
        strncpy(body->id_string, id_string.c_str(), get_sdr::body::get_id_strlen(body));
    }
