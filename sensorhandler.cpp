                             ...
                             ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    constexpr auto NON_CRITICAL_LOW_MASK = 0x1;
    constexpr auto NON_CRITICAL_HIGH_MASK = 0x8;
    constexpr auto CRITICAL_LOW_MASK = 0x2;
    constexpr auto CRITICAL_HIGH_MASK = 0x10;
    ...
    try
    {
        ...
        responseData->lowerNonCritical = static_cast<uint8_t>((
            warnLow - sensorInfo.scaledOffset)/coefficientM);

        responseData->upperNonCritical = static_cast<uint8_t>((
            warnHigh - sensorInfo.scaledOffset)/coefficientM);

        //Mark non critcal values as valid.
    ...
    try
    {
        ...
        responseData->lowerCritical = static_cast<uint8_t>(
            (critLow - sensorInfo.scaledOffset)/coefficientM);

        responseData->upperNonCritical = static_cast<uint8_t>(
            (critHigh - sensorInfo.scaledOffset)/coefficientM);
    ...
    {
        //Mask critical values if the property is not present.
        responseData->validMask &= ~(CRITICAL_LOW_MASK);
        responseData->validMask &= ~(CRITICAL_HIGH_MASK);
    }
