...
ipmi::RspType<uint8_t, // respcount
              ...
              >
    ipmiSensorGetSdrInfo(std::optional<uint8_t> count)
{
    uint8_t sdrCount;
    uint8_t lunsAndDynamicPopulation;

    if (count != 0 && get_sdr_info::request::get_count(&count) == false)
    {
    ...
    response::set_lun_present(0, lunsAndDynamicPopulation);
    response::set_lun_not_present(1, lunsAndDynamicPopulation);
    response::set_lun_not_present(2, lunsAndDynamicPopulation);
    response::set_lun_not_present(3, lunsAndDynamicPopulation);
    response::set_static_population(lunsAndDynamicPopulation);
...
ipmi::RspType<uint16_t> ipmiSensorReserveSdr()
{
    ...
    return ipmi::responseSuccess(reservation_id);
}
...
void register_netfn_sen_functions()
{
    ...
    ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::netFnSensor,
                          ...
                          ipmi::Privilege::User, ipmiSensorGetSdrInfo);
