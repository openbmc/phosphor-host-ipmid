 ...
 *   - operation        - sensor scanning status / reading state.
 *   - assertOffset0_7  - discrete assertion states(0-7).
 *   - assertOffset8_14 - discrete assertion states(8-14).
 */
ipmi::RspType<uint8_t, // sensor reading
              uint8_t, // sensor scanning status / reading state.
              uint8_t, // discrete assertion states(0-7).
              uint8_t  // discrete assertion states(7-14)
              >
    ipmiSensorGetSensorReading(uint8_t sensorNum)
{
    ...
    try
    {
        ...
        scanReadState = 1 << scanningEnabledBit;

        return ipmi::responseSuccess(senReading, scanReadState,
...
using SensorThresholds =
    std::tuple<uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t, uint8_t>;

SensorThresholds getSensorThresholds(uint8_t sensorNum)
{
    uint8_t validMask;
    ...
    if (warnLow != 0)
    {
        ...
        lowerNonCritical = static_cast<uint8_t>((warnLow - info.scaledOffset) /
                                                info.coefficientM);
        validMask |= static_cast<uint8_t>(
    ...
    if (warnHigh != 0)
    {
        ...
        upperNonCritical = static_cast<uint8_t>((warnHigh - info.scaledOffset) /
                                                info.coefficientM);
        validMask |= static_cast<uint8_t>(
    ...
    if (critLow != 0)
    {
        ...
        lowerCritical = static_cast<uint8_t>((critLow - info.scaledOffset) /
                                             info.coefficientM);
        validMask |= static_cast<uint8_t>(
    ...
    if (critHigh != 0)
    {
        ...
        upperCritical = static_cast<uint8_t>((critHigh - info.scaledOffset) /
                                             info.coefficientM);
        validMask |= static_cast<uint8_t>(
            ipmi::sensor::ThresholdMask::CRITICAL_HIGH_MASK);
    }

    return SensorThresholds{validMask,          lowerNonCritical,
                            lowerCritical,      lowerNonRecoverable,
 ...
 */
ipmi::RspType<SensorThresholds> ipmiSensorGetSensorThresholds(uint8_t sensorNum)
{
    ...
    try
    {
        return ipmi::responseSuccess(getSensorThresholds(sensorNum));
    }
    ...
    constexpr uint8_t validMask = 0;
    constexpr uint8_t lowerNonCritical = 0;
    constexpr uint8_t lowerCritical = 0;
    constexpr uint8_t lowerNonRecoverable = 0;
    constexpr uint8_t upperNonCritical = 0;
    constexpr uint8_t upperCritical = 0;
    constexpr uint8_t upperNonRecoverable = 0;

    return ipmi::responseSuccess(SensorThresholds{
        validMask, lowerNonCritical, lowerCritical, lowerNonRecoverable,
