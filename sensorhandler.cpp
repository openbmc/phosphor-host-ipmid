    ...
    ipmiSensorGetDeviceSdrInfo(std::optional<uint8_t> count)
{
    ...
    constexpr uint8_t GET_SDR_COUNT = 0x01;
    constexpr uint8_t GET_SENSOR_COUNT = 0x00;

    if (count.value_or(0) == GET_SDR_COUNT)
    {
    ...
    else if (count.value_or(0) == GET_SENSOR_COUNT)
    {
    ...
    {
        log<level::ERR>("reserved space was not set to 0");
    }
