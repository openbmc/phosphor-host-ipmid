...
ipmi::sensor::Value formatVariant(uint8_t reading, const std::string& type) {
    if (type == "int" || type == "int64_t") {
        return ipmi::sensor::Value(static_cast<int64_t>(reading));
    } else if (type == "uint" || type == "uint64_t") {
        return ipmi::sensor::Value(static_cast<uint64_t>(reading));
    } else if (type == "byte" || type == "uint8_t" || type == "char") {
        return ipmi::sensor::Value(static_cast<uint8_t>(reading));
    } else {
...
ipmi_ret_t setSensorReading(void *request)
{
    ...
    if (interfaces.empty())
    {
        log<level::ERR>("No interfaces to report, could not set sensor value.");
        return IPMI_CC_UNSPECIFIED_ERROR;
    }
    try
    {
        ...
        auto iface = "xyz.openbmc_project.Inventory.Manager";
        auto operation = "Notify";
