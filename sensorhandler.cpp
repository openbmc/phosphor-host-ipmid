...
ipmi_ret_t setSensorReading(void *request)
{
    ...
    for (const auto& interface : interfaceList)
    {
        for (const auto& property : interface.second)
        {
            ...
            bool valid = false;
            ipmi::sensor::Value reading = (int64_t)cmdData->reading;
            //if(interface.first == "xyz.openbmc_project.Sensor.Value") {
            //    props.emplace("Value", reading);
            //    valid = true;
            //}
    ...
    try
    {
        ...
        auto pimMsg = bus.new_method_call(a.bus,
                                          ...
                                          "Set");
        printf("got pimMsg\n");
        //pimMsg.append(std::move(objects.begin()->second));
        pimMsg.append("xyz.openbmc_project.Sensor.Value");
        pimMsg.append("Value");
