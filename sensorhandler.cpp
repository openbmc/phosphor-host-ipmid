...
ipmi_ret_t setSensorReading(void *request)
{
    ...
    ValueObjectMap valueObjects;
    ValueInterfaceMap valueInterfaces;
    ReadingObjectMap readingObjects;
    ReadingInterfaceMap readingInterfaces;

    for (const auto& interface : interfaceList)
    {
        if(interface.first == "xyz.openbmc_project.Sensor.Value") {
            printf("It's a Sensor.Value and we're writing %x\n",
                   cmdData->reading);

            ReadingPropertyMap analogProperties;
            // TODO: that int should be instead the property from the map
            Reading reading = cmdData->reading;
            analogProperties.emplace("Value", reading);
            printf("emplaced into analogProperties %s, %x, analogProperties.size() = %d\n",
                   "int", cmdData->reading, analogProperties.size());
            readingInterfaces.emplace(interface.first, analogProperties);
            printf("did the emplace into readingInterfaces %s, analogProperties; readingInterfaces.size() = %d\n",
                   interface.first.c_str(), readingInterfaces.size());
        }
        for (const auto& property : interface.second)
        {
            printf("property: %s\n", property.first.c_str());
            ValuePropertyMap props;
            bool valid = false;
            ...
            if (valid)
            {
                valueInterfaces.emplace(interface.first, std::move(props));
            }
        }
    }
    valueObjects.emplace(iter->second.sensorPath, std::move(valueInterfaces));
    readingObjects.emplace(iter->second.sensorPath, std::move(readingInterfaces));
    printf("valueObjects.size() = %d, readingObjects.size() = %d\n",
            valueObjects.size(), readingObjects.size());
    printf("did the emplace into *Objects\n");
    ...
    try
    {
        ...
        auto pimMsg = bus.new_method_call(a.bus,
                                          ...
                                          "Set");
        printf("got pimMsg\n");
        pimMsg.append(std::move(valueObjects));
        pimMsg.append(std::move(readingObjects));
        printf("gave it *Objects\n");
                                 ...
                                 ipmi_data_len_t data_len, ipmi_context_t context)
{
    ...
    if (request == nullptr || GET_SDR_INFO_REQ_GET_COUNT(request) == false)
    {
        // Get Sensor Count
    ...
    GET_SDR_INFO_RESP_SET_LUN_PRESENT(0, resp->luns_and_dynamic_popl);
    GET_SDR_INFO_RESP_SET_LUN_NOT_PRESENT(1, resp->luns_and_dynamic_popl);
    GET_SDR_INFO_RESP_SET_LUN_NOT_PRESENT(2, resp->luns_and_dynamic_popl);
    GET_SDR_INFO_RESP_SET_LUN_NOT_PRESENT(3, resp->luns_and_dynamic_popl);
    GET_SDR_INFO_RESP_SET_STATIC_POPL(resp->luns_and_dynamic_popl);
