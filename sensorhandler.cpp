...
ipmi_ret_t setSensorReading(void *request)
{
    ...
    for (const auto& interface : interfaceList)
    {
        ...
        for (const auto& property : interface.second)
        {
            ...
            for (const auto& readingTypes : property.second)
            {
                ...
                {
                    if(readingTypes.first.compare("reading") == 0)
                    {
                    ...
                    {
                        if(readingTypes.first.compare("assertion") == 0)
                        {
                        ...
                        else if(readingTypes.first.compare("reading") == 0)
                        {
                            if(0xFF == value.first)
                            {
    ...
    try
    {
        ...
        {
            log<level::ERR>("Error in dbus call");
            return IPMI_CC_UNSPECIFIED_ERROR;
        }
    }
    catch(const std::runtime_error& e)
