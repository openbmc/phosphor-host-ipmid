                  ...
                  Info sensorInfo)
{
    ...
    for (const auto& interface : interfaceList)
    {
        ...
        for (const auto& property : interface.second)
        {
            ...
            {
                updMsg.append(sdbusplus::message::variant<uint32_t>(setVal));
                break;
            }
            else if (rtype == IPMI_TYPE_ASSERTION)
            {
                for (const auto& value : property.second)
                {
                    if (assertionSet.test(value.first))
            ...
            else
            {
                ...
                updMsg.append(iter->second.assert);
                        log<level::ERR>(iter->second.assert.get<std::string>().c_str());
            }
        }
    }
...
{3,{
        ...
        0,IPMI_TYPE_EVENT2,sensor_set::Set::update,std::bind(getValue, ipmi::sensor::IPMI_TYPE_EVENT2, std::placeholders::_1),{
...
{5,{
        ...
        0,IPMI_TYPE_ASSERTION,sensor_set::Set::update,std::bind(getValue, ipmi::sensor::IPMI_TYPE_ASSERTION, std::placeholders::_1),{
...
{7,{
        ...
        0,IPMI_TYPE_READING,sensor_set::Set::update,std::bind(getValue, ipmi::sensor::IPMI_TYPE_READING, std::placeholders::_1),{
