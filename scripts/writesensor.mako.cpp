...
<%
    ...
    if serviceInterface not in funcProps:
        funcProps[serviceInterface].append(command)
        funcProps[serviceInterface].append(updateFunc)
    for interface,properties in sensor["interfaces"].items():
        for dbus_property,property_value in properties.items():
            for offset,values in property_value.iteritems():
                if offset == 0xFF:
                    funcProps[serviceInterface].append(values["type"])
%>\
% endfor
#include <bitset>
#include "types.hpp"
    ...
    for (const auto& interface : interfaceList)
    {
        ...
        for (const auto& property : interface.second)
        {
            ...
            {   //if the offset is FF then send the value as is
                ...
                {
                    updMsg.append(sdbusplus::message::variant<${funcProp[2]}>(setVal));
                }
                else
                {
