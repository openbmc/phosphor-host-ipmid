...
void ChannelConfig::processChAccessPropChange(const std::string& path,
    ...
    for (auto it = data.begin(); it != data.end(); ++it)
    {
        try
        {
            ...
            if (!itValue.is_null())
            {
                if (chNum > maxIpmiChannels)
                {
                    log<level::ERR>("Invalid IPMI channel ID in the configuration file.",
            ...
            {
                log<level::WARNING>("Channel not configured so loading default.",
                                    entry("CHANNEL_NUM:%d", chNum));
