...
static constexpr const char *channelNvDataFilename =
    "/var/ipmi/channel_access_nv.json";
static constexpr const char *channelVolatileDataFilename =
    "/tmp/ipmi/channel_access_volatile.json";
...
int ChannelConfig::getChannelActiveSessions(const uint8_t &chNum)
{
    ...
    int chActSessions = channelData[chNum].activeSessCount;
    return chActSessions;
}
                                                    ...
                                                    uint8_t &authType)
{
    ...
    if (getChannelSessionSupport(chNum) == sessionNone)
    {
        log<level::DEBUG>("Session-les channel doesn't have access data.");
        return IPMI_CC_INVALID_FIELD_REQUEST;
    }
