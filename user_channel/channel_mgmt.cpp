...
std::string ChannelConfig::convertToIntfNameFromChannelNum(const int chNum)
{
    ...
    log<level::ERR>("Invalid channel number.", entry("ChannelID:%d", chNum));
...
std::string
    ChannelConfig::convertToChannelNameFromIntfName(const std::string& intfName)
{
    for (const auto& it : channelData)
    {
...
int ChannelConfig::convertToChannelNumberFromIntfName(
...
int ChannelConfig::loadChannelConfig()
{
    ...
    for (auto it = data.begin(); it != data.end(); ++it)
    {
        try
        {
            std::string itKey = it.key();
            chNum = itKey.empty() ? 0 : std::stoi(itKey);
            Json itValue = it.value();
            if (!itValue.is_null())
            {
                if (chNum > maxIpmiChannels)
                {
                    log<level::ERR>(
                        "Invalid IPMI channel ID in the configuration file.",
                ...
                chData->activeSessCount =
                    itValue.value(activeSessionsString, 0);
                ...
                chData->chInfo.protocolType = static_cast<uint8_t>(
