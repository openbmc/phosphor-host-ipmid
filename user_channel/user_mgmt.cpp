                      ...
                      const bool& enabled, const std::string& newUserName)
{
    ...
    else
    {
        ...
        {
            lg2::debug(
                "User not found for signal, user name: {USER_NAME}, user event: {USER_EVENT}",
    ...
    lg2::debug(
        "User event handled successfully, user name: {USER_NAME}, user event: {USER_EVENT}",
...
bool UserAccess::isValidUserName(const std::string& userName)
{
    ...
    {
        lg2::debug("{USER_NAME} already exists", "USER_NAME", userName);
        return false;
    }
...
uint8_t UserAccess::getUserId(const std::string& userName)
{
    ...
    if (usrIndex > ipmiMaxUsers)
    {
        lg2::debug("{USER_NAME} not found", "USER_NAME", userName);
                              ...
                              const std::string& sysPriv, const bool& enabled)
{
    ...
    for (size_t usrIndex = 1; usrIndex <= ipmiMaxUsers; ++usrIndex)
    {
        ...
        if (userName == curName)
        {
            lg2::debug("{USER_NAME} exists", "USER_NAME", userName);
