...
int PasswdMgr::checkPasswdFile(void)
{
    ...
    ssize_t total_len = sizeof(*metaData) + metaData->hashSize + metaData->ivSize
...
int PasswdMgr::createPasswdFile(void)
{
    ...
    bytesWritten = std::snprintf(
            reinterpret_cast<char*>(&inBytes[0]),
            inBytesLen, "%s:%s\n",
            userName.c_str(),
            passWord.c_str());
    ...
    fprintf(stderr,"Total len:%ld\n",sizeof(metaData)+metaData.hashSize+metaData.ivSize+metaData.dataSize+metaData.padSize+metaData.macSize);
