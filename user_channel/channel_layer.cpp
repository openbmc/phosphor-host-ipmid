...
ipmi_ret_t getChannelAccessPersistData(const uint8_t chNum,
                                       ChannelAccess& chAccessData)
{
    return channelConfig->getChannelAccessPersistData(chNum,
                                                      chAccessData);
}
...
ipmi_ret_t setChannelAccessPersistData(const uint8_t chNum,
                                       ...
                                       const uint8_t setFlag)
{
    return channelConfig->setChannelAccessPersistData(
...
{
    return channelConfig->getChannelAuthTypeSupported(
...
ipmi_ret_t getChannelEnabledAuthType(const uint8_t chNum, const uint8_t priv,
                                     EAuthType& authType)
{
    return channelConfig->getChannelEnabledAuthType(chNum, priv,
                                                    authType);
}

std::string getChannelIntfNameFromChannelNumber(const int chNum)
{
    return channelConfig->convertToIntfNameFromChannelNum(chNum);
}
