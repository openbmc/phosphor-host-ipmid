...
ipmi_ret_t ipmi_get_user_access(ipmi_netfn_t netfn, ipmi_cmd_t cmd,
  ...
  *data_len = 0;
  ...
  if (!UserAccess::isValidUserId(req->user_id) ||
      !UserAccess::isValidChannel(req->ch_num)) {
  ...
  resp->enabled_status = USER_ID_DISABLED_VIA_SET_PASSWORD >>
                         userData->user[req->user_id].userEnabled;
  resp->privilege =
      userData->user[req->user_id].userPrivAccess[ch_num].privilege;
  resp->ipmi_enabled =
      userData->user[req->user_id].userPrivAccess[ch_num].ipmi_enabled;
  resp->link_auth_enabled =
      userData->user[req->user_id].userPrivAccess[ch_num].link_auth_enabled;
  resp->access_callback =
      userData->user[req->user_id].userPrivAccess[ch_num].access_callback;
...
ipmi_ret_t ipmi_set_user_name(ipmi_netfn_t netfn, ipmi_cmd_t cmd,
                              ...
                              ipmi_data_len_t data_len,
                              ipmi_context_t context) {
  const set_user_name_req_t *req = static_cast<set_user_name_req_t *>(request);
  size_t req_length = *data_len;
  *data_len = 0;
  ...
  if (ret == INVALID_USER_NAME) {
    ...
    log<level::DEBUG>("Set user name - Internal error");
...
ipmi_ret_t ipmi_get_user_name(ipmi_netfn_t netfn, ipmi_cmd_t cmd,
                              ...
                              ipmi_data_len_t data_len,
                              ipmi_context_t context) {
  const get_user_name_req_t *req = static_cast<get_user_name_req_t *>(request);
  size_t req_length = *data_len;
  ...
  *data_len = 0;
  ...
  if (userAccess.getUserName(req->user_id, user_name) !=
      0) {  // Invalid User ID
...
bool pam_update_passwd(const char *username, const char *password) {
  ...
  if (pam_start("passwd", username, &local_conversation, &local_auth_handle) !=
      PAM_SUCCESS) {
    return false;
  }
...
ipmi_ret_t ipmi_set_user_password(ipmi_netfn_t netfn, ipmi_cmd_t cmd,
  ...
  *data_len = 0;
  ...
  if (((req->operation == SET_PASSWORD) || (req->operation == TEST_PASSWORD)) &&
  ...
  if (req->operation == SET_PASSWORD) {
    ...
    log<level::ERR>("Other operations not implemented - TODO yet to implement");
    return IPMI_CC_INVALID_FIELD_REQUEST;
  }
  return IPMI_CC_OK;
}
