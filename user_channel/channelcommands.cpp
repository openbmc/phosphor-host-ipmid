...
ipmi::RspType<> ipmiSetChannelAccess(ipmi::Context::ptr ctx, uint4_t channel,
                                     uint4_t reserved1, uint2_t accessMode,
...
ipmi ::RspType<uint3_t, // access mode,
               ...
               uint4_t  // reserved
               >
    ipmiGetChannelAccess(ipmi::Context::ptr ctx, uint4_t channel,
                         uint4_t reserved1, uint6_t reserved2,
                         uint2_t accessSetMode)
{
    const uint8_t chNum =
        convertCurrentChannelNum(static_cast<uint8_t>(channel));
    ...
    if (EChannelSessSupported::none == getChannelSessionSupport(chNum))
    {
        ...
        return ipmi::response(IPMI_CC_ACTION_NOT_SUPPORTED_FOR_CHANNEL);
    }
...
void registerChannelFunctions()
{
    ...
    ipmi::registerHandler(ipmi::prioOpenBmcBase, ipmi::netFnApp,
                          ipmi::app::cmdGetChannelAccess, ipmi::Privilege::User,
