...
/** @brief create user with groups, privilege and enabled state
 *
 *  @param[in] userId - user id
 *  @param[in] userName - user name
 ...
 *  @return ccSuccess for success, others for failure.
 */
Cc ipmiUserCreateUser(const uint8_t userId, const std::string& userName,
...
/** @brief rename an entry in the list of None Ipmi group users
 *
 ...
 * @return ccSuccess for success, others for failure.
 */
Cc ipmiUserRenameNonIpmiGroupUser(const std::string& userName,
                                  const std::string& newUserName);
